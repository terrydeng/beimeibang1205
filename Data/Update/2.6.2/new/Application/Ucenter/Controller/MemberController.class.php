...
class MemberController extends Controller
{
    ...
    public function register()
    {
        ...
        if (IS_POST) {
            ...
            if (0 < $uid) { //注册成功
                ...
                M('Member')->login($uid, false, $aRole); //登陆
    ...
    public function saveAvatar()
    {
        ...
        $res = D('avatar')->where(array('uid' => $aUid))->save($data);
        if (!$res) {
            D('avatar')->add($data);
        }
    ...
    private function initInviteUser($uid = 0, $code = '', $role = 0)
    {
        if ($code != '') {
            ...
            if ($result) {
                ...
                if ($invite['uid'] > 0) {
                    M('Ucenter/Score')->setUserScore(array($invite['uid']), $invite_type['income_score'], $invite_type['income_score_type'], 'inc', '', 0, L('_ERROR_BONUS_'));
                }
            }
        }
        return true;
