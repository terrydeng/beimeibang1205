...
class AdminConfigBuilder extends AdminBuilder
{
    ...
    public function handleConfig()
    {
        if (IS_POST) {
            ...
            if ($success) {
                ...
                exit(json_encode(array('info' => L('_SUCCESS_CONF_SAVE_').L('PERIOD'), 'status' => 1, 'url' => __SELF__)));
            } else {
                header('Content-type: application/json');
                exit(json_encode(array('info' => L('_FAIL_CONF_SAVE_').L('PERIOD'), 'status' => 0, 'url' => __SELF__)));
            }


        } else {
            $configs = M('Config')->where(array('name' => array('like', '_' . strtoupper(CONTROLLER_NAME) . '_' . '%')))->limit(999)->select();
            $data = array();
    ...
    {
        $list = D('AuthGroup')->where(array('status' => 1))->order('id asc')->select();
        $result = array();
    ...
    public function groupLocalComment($group_name,$mod){
        ...
        $this->keyRadio($mod.'_LOCAL_COMMENT_CAN_GUEST', L('_COMMENTS_ALLOW_VISITOR_IF_'), L('_ALLOW_DEFAULT_'),array(0=>L('_DISALLOW_'),1=>L('_ALLOW_')))
            ->keyRadio($mod.'_LOCAL_COMMENT_ORDER',L('_COMMENTS_SORT_'),L('_DESC_DEFAULT_'),array(0=>L('DESC'),1=>L('ASC')))
            ->keyText($mod.'_LOCAL_COMMENT_COUNT',L('_COMMENTS_PAGE_DISPLAY_COUNT_'),L('post_show_image_number_default'));
