...
function _getAddonsConfig()
{
    ...
    if (!$config) {
        $config = D('Addons')->where(array('name' => 'Recommend'))->find();
...
function _randRecommend()
{
    ...
    if ($already_show) {
        ...
        $other_follow = M('Member')->where($map_1)->field('uid')->order('rand()')->limit(1)->select();
        ...
        $other_follow = M('Member')->where($map_2)->field('uid')->order('rand()')->limit(1)->select();
...
function _sameCityRecommend()
{
    ...
    $other_follow = M('Member')->where($map_d)->field('uid')->limit(1)->select();//符合以上条件的推荐
    $other_follow = array_column($other_follow, 'uid');
    ...
    if (count($other_follow)) {
        ...
        $other_follow = M('Member')->where($map_c)->field('uid')->limit(1)->select();//符合以上条件的推荐
        $other_follow = array_column($other_follow, 'uid');
        ...
        if (count($other_follow)) {
            ...
            $other_follow = M('Member')->where($map_p)->field('uid')->limit(1)->select();//符合以上条件的推荐
            $other_follow = array_column($other_follow, 'uid');
            if (count($other_follow)) {
                if ($already_show) {
                    $map = array_merge($other_follow, $already_show);
                ...
                $other_follow = M('Member')->where($map_o)->order('rand()')->field('uid')->limit(1)->select();//符合以上条件的推荐
                $other_follow = array_column($other_follow, 'uid');
                if ($other_follow) {
                    if ($already_show) {
                        $map = array_merge($other_follow, $already_show);
    ...
    if ($other_follow) {
        $reason = ' <sapn style="color: #808080">同城推荐</sapn>';
        $other_follow = query_user(array('uid', 'avatar64', 'nickname', 'signature', 'space_url'), $other_follow[0]);
        $other_follow['reason'] = $reason;
    }
    //输出
...
function _adminRecommend()
{
    ...
    $config = D('Addons')->where(array('name' => 'Recommend'))->find();//获取用户推荐ID
    ...
    if ($already_show) {
        ...
        $other_follow = M('Member')->where($map)->limit(1)->select();
        $other_follow = array_column($other_follow, 'uid');
        S('recommend_follow_id_' . is_login(), array_merge($already_show, $other_follow), 600);
    } else {
        $other_follow = $follow_who_ids;
...
function _sameDataRecommend()
{
    ...
    $other_follow = M('Member')->where($map_b)->field('uid')->limit(1)->select();//符合以上条件的推荐
    $other_follow = array_column($other_follow, 'uid');
...
function _followFollowId()
{
    ...
    if (count($follow_who_ids)) {
        ...
        $follow_follow_ids = M('Follow')->where($map)->field('follow_who')->select();  //where的查询条件,得到我关注的人，关注的人的ID。
        ...
        if (count($follow_follow_ids)) {
            ...
            if ($already_show) {
                ...
                $other_follow = M('Member')->where($map_1)->order('score desc')->field('uid')->limit(1)->select();
                $other_follow = array_column($other_follow, 'uid');
            ...
            if ($already_show) {
                ...
                $other_follow = M('Member')->where($map_2)->order('score desc')->field('uid')->limit(1)->select();
                $other_follow = array_column($other_follow, 'uid');
                if (!is_null($other_follow)) {
                    S('recommend_follow_id_' . is_login(), array_merge($already_show, $other_follow), 600);
                }
