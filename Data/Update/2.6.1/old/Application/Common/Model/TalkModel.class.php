...
class TalkModel extends Model
{
    ...
    public function getCurrentSessions()
    {
        ...
        $new_talks = M('TalkPush')->where(array('uid' => get_uid(), 'status' => array('NEQ', -1)))->select();
        $new_ids = array();
        foreach ($new_talks as $push) {
            M('TalkPush')->where(array('id' => $push['id']))->setField('status', 1);//全部置为已提示
            $new_ids[] = $push['source_id'];
        }


        //每次获取到所有的id，就对这些做delete处理。防止反复提示。
        $new_talk_messages = M('TalkMessagePush')->where(array('uid' => get_uid(), 'status' => array('NEQ', -1)))->select();
        foreach ($new_talk_messages as $v) {
            M('TalkMessagePush')->where(array('id' => $v['id']))->setField('status', 1);//全部置为已提示
            $message = M('TalkMessage')->find($v['source_id']);
    ...
    {
        $last_message = M('TalkMessage')->where('talk_id=' . $talk_id)->order('create_time desc')->find();
