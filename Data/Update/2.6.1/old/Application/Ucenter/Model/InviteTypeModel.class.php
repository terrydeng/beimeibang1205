...
class InviteTypeModel extends Model
{
    ...
    public function getUserTypeSimpleList($field = 'id,title'){
        $group_ids=M('AuthGroupAccess')->where(array('uid'=>is_login()))->field('group_id')->select();
    ...
    public function getUserTypeList()
    {
        ...
        $scoreTypeModel=M('UcenterScoreType');
        $roleModel=M('Role');
        $inviteUserInfoModel=M('Ucenter/InviteUserInfo');
    ...
    private function _initSelectData($data = array())
    {
        ...
        $scoreTypes = M('UcenterScoreType')->where(array('id' => array('in', array($data['pay_score_type'], $data['income_score_type']))))->field('id,title,unit')->select();
        $scoreTypes = array_combine(array_column($scoreTypes, 'id'), $scoreTypes);
        ...
        if ($data['roles'] != '') {
            ...
            $role_list = M('Role')->where(array('id' => array('in', $data['roles_show'])))->field('id,title')->select();
            $role_list = array_combine(array_column($role_list, 'id'), $role_list);
        ...
        if ($data['auth_groups'] != '') {
            ...
            $auth_group_list = M('AuthGroup')->where(array('id' => array('in', $data['auth_groups_show'])))->field('id,title')->select();
