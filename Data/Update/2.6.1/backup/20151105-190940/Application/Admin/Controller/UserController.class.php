...
class UserController extends AdminController
{
    ...
    public function changeGroup()
    {
        ...
        if ($_POST['do'] == 1) {
            ...
            if ($aAll) {//设置全部用户
                $prefix = C('DB_PREFIX');
                M('')->execute("TRUNCATE TABLE {$prefix}auth_group_access");
                $aUids = UCenterMember()->getField('id', true);

            } else {
                D('AuthGroupAccess')->where(array('uid' => array('in', implode(',', $aUids))))->delete();;
            }
            foreach ($aUids as $uid) {
                foreach ($aGids as $gid) {
                    D('AuthGroupAccess')->add(array('uid' => $uid, 'group_id' => $gid));
                }
            }
            ...
            $groups = D('AuthGroup')->where(array('status' => 1))->select();
    ...
    public function expandinfo_select($page = 1, $r = 20)
    {
        ...
        $list = D('Member')->where($map)->order('last_login_time desc')->page($page, $r)->select();
        $totalCount = D('Member')->where($map)->count();
        int_to_string($list);
        //扩展信息查询
        $map_profile['status'] = 1;
        $field_group = M('field_group')->where($map_profile)->select();
        $field_group_ids = array_column($field_group, 'id');
        $map_profile['profile_group_id'] = array('in', $field_group_ids);
        $fields_list = M('field_setting')->where($map_profile)->getField('id,field_name,form_type');
        ...
        foreach ($list as &$tkl) {
            ...
            foreach ($fields_list as $key => $val) {
                ...
                $field_data = M('field')->where($map_field)->getField('field_data');
    ...
    public function expandinfo_details($uid = 0)
    {
        if (IS_POST) {
            ...
            unset($key, $val);
            $res = M('Member')->where(array('uid' => $data['id']))->save($data_score);
            foreach ($data_score as $key => $val) {
                $value = query_user(array($key), $data['id']);
                if ($val == $value[$key]) {
                    continue;
                }
                M('Ucenter/Score')->addScoreLog($data['id'], cut_str('score', $key, 'l'), 'to', $val, '', 0, get_nickname(is_login()) . L('_BACKGROUND_ADJUSTMENT_'));
                M('Ucenter/Score')->cleanUserCache($data['id'], cut_str('score', $key, 'l'));
            }
            unset($key, $val);
            /* 修改积分 end*/
            /*身份设置 zzl(郑钟良)*/
            ...
            $map['uid'] = $uid;
            $map['status'] = array('egt', 0);
            $member = D('Member')->where($map)->find();
            ...
            /* 积分设置 xjw129xjt(肖骏涛)*/
            $field = M('Ucenter/Score')->getTypeList(array('status' => 1));
            $score_key = array();
            foreach ($field as $vf) {
                $score_key[] = 'score' . $vf['id'];
                $builder->keyText('score' . $vf['id'], $vf['title']);
            }
            $score_data = M('Member')->where(array('uid' => $uid))->field(implode(',', $score_key))->find();
            $member = array_merge($member, $score_data);
            /*积分设置end*/
            $builder->data($member);

            /*身份设置 zzl(郑钟良)*/
            $already_role = M('UserRole')->where(array('uid' => $uid, 'status' => 1))->field('role_id')->select();
    ...
    {
        $map['status'] = array('egt', 0);
        $profileList = M('field_group')->where($map)->order("sort asc")->page($page, $r)->select();
        $totalCount = M('field_group')->where($map)->count();
        $builder = new AdminListBuilder();
    ...
    public function sortProfile($ids = null)
    {
        if (IS_POST) {
            ...
            $map['status'] = array('egt', 0);
            $list = M('field_group')->where($map)->order("sort asc")->select();
    ...
    {
        $profile = M('field_group')->where('id=' . $id)->find();
        $map['status'] = array('egt', 0);
        $map['profile_group_id'] = $id;
        $field_list = M('field_setting')->where($map)->order("sort asc")->page($page, $r)->select();
        $totalCount = M('field_setting')->where($map)->count();
        $type_default = array(
    ...
    public function sortField($id = '', $ids = null)
    {
        if (IS_POST) {
            ...
            $map['status'] = array('egt', 0);
            $map['profile_group_id'] = $id;
            $list = M('field_setting')->where($map)->order("sort asc")->select();
            foreach ($list as $key => $val) {
    ...
    public function editFieldSetting($id = 0, $profile_group_id = 0, $field_name = '', $child_form_type = 0, $visiable = 0, $required = 0, $form_type = 0, $form_default_value = '', $validation = 0, $input_tips = '')
    {
        if (IS_POST) {
            ...
            $roleOptions = M('Role')->selectByMap(array('status' => array('gt', -1)), 'id asc', 'id,title');

            $builder = new AdminConfigBuilder();
            if ($id != 0) {
                $field_setting = M('field_setting')->where('id=' . $id)->find();
    ...
    public function changeProfileStatus($status)
    {
        ...
        M('field_group')->where(array('id' => array('in', $id)))->setField('status', $status);
        if ($status == -1) {
    ...
    public function action()
    {
        ...
        //获取列表数据
        $Action = D('Action')->where(array('status' => array('gt', -1)));
    ...
    {
        $scoreTypes = M('Ucenter/Score')->getTypeList(array('status' => 1));
        if (IS_POST) {
            $aUids = I('post.uid');
            foreach ($scoreTypes as $v) {
                $aAction = I('post.action_score' . $v['id'], '', 'op_t');
                $aValue = I('post.value_score' . $v['id'], 0, 'intval');
                M('Ucenter/Score')->setUserScore($aUids, $aValue, $v['id'], $aAction, '', 0, L('_BACKGROUND_ADMINISTRATOR_RECHARGE_PAGE_RECHARGE_'));
                M('Ucenter/Score')->cleanUserCache($aUids, $aValue);
