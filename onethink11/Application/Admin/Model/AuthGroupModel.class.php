...
class AuthGroupModel extends Model {
    ...
    public function addToGroup($uid,$gid){
        ...
        $Access = D(self::AUTH_GROUP_ACCESS);
        ...
        if( $del!==false ){
            foreach ($uid_arr as $u){
            	//判断用户id是否合法
            	if(D('Member')->getFieldByUid($u,'uid') == false){
    ...
    static public function getExtendOfGroup($gid,$type){
        ...
        return D(self::AUTH_EXTEND)->where( array('group_id'=>$gid,'type'=>$type) )->getfield('extend_id',true);
    }
    ...
    public function removeFromGroup($uid,$gid){
        return D(self::AUTH_GROUP_ACCESS)->where( array( 'uid'=>$uid,'group_id'=>$gid) )->delete();
    }
    ...
    static public function memberInGroup($group_id){
        ...
        $list     = D() ->field('m.uid,u.username,m.last_login_time,m.last_login_ip,m.status')
                       ->table($l_table.' m')
    ...
    public function checkId($modelname,$mid,$msg = '以下id不存在:'){
        ...
        $s = D($modelname)->where(array('id'=>array('IN',$ids)))->getField('id',true);
        if(count($s)===$count){
            return true;
        }else{
