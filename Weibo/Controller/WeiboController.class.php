...
class WeiboController extends AdminController
{
    ...
    public function config()
    {
        ...
        $builder->title(L('_WEIBO_BASIC_SETTINGS_'))
            ...
            //->keySelect('WEIBO_DEFAULT_TAB', '轻博客默认显示标签', '', array('all'=>'全站轻博客','concerned'=>'我的关注','hot'=>'热门轻博客'))
    ...
    {
        foreach ($ids as $id) {
            M('Weibo')->where(array('id' => $id))->setField('is_top', $top);
            S('weibo_' . $id, null);
        }
    ...
    {
        foreach ($ids as $id) {
            $comemnt = M('Weibo/WeiboComment')->getComment($id);
            if ($status == 1) {
                M('Weibo/Weibo')->where(array('id' => $comemnt['weibo_id']))->setInc('comment_count');
            } else {
                M('Weibo/Weibo')->where(array('id' => $comemnt['weibo_id']))->setDec('comment_count');
            }
    ...
    {
        D('WeiboTopic')->where(array('id' => array('in', $ids)))->setField('is_top', $top);
    ...
    public function delTopic($ids)
    {
        ...
        D('WeiboTopic')->where(array('id' => array('in', $ids)))->delete();
        S('topic_rank', null, 60);
