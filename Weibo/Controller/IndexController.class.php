...
class IndexController extends BaseController
{
    ...
    {
        if ($aType == 'concerned') {
            $followList = M('Follow')->getFollowList();
    ...
    public function doSendRepost()
    {
        ...
        if ($new_id) {
            M('weibo')->where('id=' . $aSourceId)->setInc('repost_count');
            $aWeiboId != $aSourceId && M('weibo')->where('id=' . $aWeiboId)->setInc('repost_count');
...
// 发送消息
        ...
        M('Common/Message')->sendMessage($toUid, L('_PROMPT_TRANSMIT_'), $user['nickname'] .L('_TIP_TRANSMITTED_').L('_EXCLAMATION_'), 'Weibo/Index/weiboDetail', array('id' => $new_id), is_login(), 1);
    ...
    public function commentlist()
    {
        ...
        $list = M('WeiboComment')->getCommentList($aWeiboId, $aPage, $aShowMore);
        $this->assign('list', $list);
        $this->assign('page', $aPage);
        $this->assign('weiboId', $aWeiboId);
        $weobo = M('Weibo')->getWeiboDetail($aWeiboId);
    ...
    public function doDelComment()
    {
        ...
        $aCommentId = I('post.comment_id', 0, 'intval');
        $this->checkIsLogin();
        $comment = M('Weibo/WeiboComment')->getComment($aCommentId);
        $this->checkAuth(null, $comment['uid'], L('_INFO_AUTHORITY_COMMENT_DELETE_LACK_').L('_PERIOD_'));


        //删除评论
        $result = M('Weibo/WeiboComment')->deleteComment($aCommentId);
    ...
    public function weiboDetail($id)
    {
        ...
        $weibo = M('Weibo')->getWeiboDetail($id);
        if ($weibo === null) {
        ...
        $supported = M('Weibo')->getSupportedPeople($weibo['id'], array('nickname', 'space_url', 'avatar128', 'space_link'), 12);
    ...
    public function search()
    {
        ...
        $param['count'] = $r;
        //查询
        $list = M('Weibo')->getWeiboList($param);
        $totalCount = M('Weibo')->where($param['where'])->count();
