...
function send_weibo($content,$type,$feed_data = '', $from = ''){
    ...
    M('Weibo/Topic')->addTopic($content);
...
function send_comment($weibo_id, $content, $comment_id = 0){
    ...
    send_comment_message($weibo['uid'], $weibo_id, L('_COMMENT_CONTENT_').L('COLON')."$content");
    //通知回复的人
    if ($comment_id) {
        $comment = M('WeiboComment')->getComment($comment_id);
        if ($comment['uid'] != $weibo['uid']) {
            send_comment_message($comment['uid'], $weibo_id, L('_REPLY_CONTENT_').L('COLON')."$content");
        }
    }
...
function send_comment_message($uid, $weibo_id, $message){
    ...
    $from_uid = is_login();
    $type = 1;
    M('Common/Message')->sendMessage($uid,$title, $message,  'Weibo/Index/weiboDetail',array('id' => $weibo_id), $from_uid, $type);
}
