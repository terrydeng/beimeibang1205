...
class AddonsController extends AdminController {
    ...
    public function create(){
        if(!is_writable(BeiMeiBang_ADDON_PATH))
...
str;
    ...
    public function checkForm(){
        ...
        if(!$data['info']['name'])
            $this->error('插件标识必须');
        //检测插件名是否合法
        $addons_dir             =   BeiMeiBang_ADDON_PATH;
...
str;
    ...
    public function sql_split($sql, $tablepre) {
        if ($tablepre != "BeiMeiBang_")
            $sql = str_replace("BeiMeiBang_", $tablepre, $sql);
    ...
    public function install(){
        ...
        if($addonsModel->add($data)){
            ...
            $hooks_update   =   M('Hooks')->updateHooks($addon_name);
            if($hooks_update){
                S('hooks', null);
                $this->success('安装成功');
            }else{
    ...
    public function uninstall(){
        ...
        $hooks_update   =   M('Hooks')->removeHooks($db_addons['name']);
    ...
    public function hooks(){
        ...
        $map    =   $fields =   array();
        $list   =   $this->lists(M("Hooks")->field($fields),$map);
    ...
    public function edit($name, $id = 0){
        ...
        $this->assign('title', $addon->info['title']);
        if(isset($model)){
            $addonModel = M("Addons://{$name}/{$model}");
            if(!$addonModel)
                $this->error('模型无法实列化');
