...
class ArticleController extends AdminController {
    ...
    protected function checkDynamic(){
        ...
        switch(strtolower(ACTION_NAME)){
            ...
            case 'permit':    //回收站
                $doc_id  =  (array)I('ids');
                $cate_id =  D('Document')->where(array('id'=>array('in',$doc_id)))->getField('category_id',true);
    ...
    protected function getDocumentList($cate_id=0,$model_id=null,$position=null,$field=true,$group_id=null){
        ...
        if ( isset($_GET['nickname']) ) {
            $map['uid'] = D('Member')->where(array('nickname'=>I('nickname')))->getField('uid');
        }
        ...
        if(!is_null($model_id)){
            ...
            if(is_array($field) && array_diff($Document->getDbFields(),$field)){
                $modelName  =   D('Model')->getFieldById($model_id,'name');
    ...
    public function edit(){
        ...
        // 获取详细数据 
        $Document = M('Doc0s1'); //$Document = M('Doc0s1A1');
    ...
    public function autoSave(){
        $res = M('Doc0s1')->autoSave(); //$res = M('Doc0s1A1')->autoSave();
        if($res !== false){
            $return['data']     =   $res;
            $return['info']     =   '保存草稿成功';
            $return['status']   =   1;
            ...
            $this->error('保存草稿失败：'.M('Doc0s1')->getError()); //$this->error('保存草稿失败：'.M('Doc0s1A1')->getError());
        }
    }
    ...
    public function clear(){
        $res = M('Doc0s1')->remove(); //$res = M('Doc0s1A1')->remove();
        if($res !== false){
    ...
    protected function checkPaste($list, $cate_id, $pid){
        ...
        // 检查支持的文档模型
        $modelList =   D('Category')->getFieldById($cate_id,'model');   // 当前分类支持的文档模型
        ...
        // 检查支持的文档类型和层级规则
        $typeList =   D('Category')->getFieldById($cate_id,'type'); // 当前分类支持的文档模型
        foreach ($list as $key=>$value){
            // 移动文档的所属文档模型
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = D('Document')->where($map)->field('id,title')->order('level DESC,id DESC')->select();
            ...
            foreach ($ids as $key=>$value){
                $res = D('Document')->where(array('id'=>$value))->setField('level', $key+1);
            }
            if($res !== false){
                $this->success('排序成功！');
            }else{
