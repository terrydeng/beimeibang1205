...
class CategoryController extends AdminController {
    ...
    public function index(){
        $tree = M('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,status');
    ...
    public function remove(){
        ...
        $child = D('Category')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
            $this->error('请先删除该分类下的子分类');
        }

        //判断该分类下有没有内容
        $document_list = D('Document')->where(array('category_id'=>$cate_id))->field('id')->select();
    ...
    public function operate($type = 'move'){
        ...
        $list = D('Category')->where($map)->field('id,pid,title')->select();
        ...
        if(strcmp($type, 'move') == 0){
        	...
        	$pid = D('Category')->getFieldById($from, 'pid');
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = D('Category')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
    ...
    public function merge(){
        ...
        //合并文档
        $res = D('Document')->where(array('category_id'=>$from))->setField('category_id', $to);
