...
class Article extends TagLib{
    ...
    public function _list($tag, $content){
        ...
        $parse  = '<?php ';
//        $parse .= '$__CATE__ = M(\'Category\')->getChildrenId('.$cate.');';
        $parse .= '$__CATE__ = M(\'Cat0s1\')->getChildrenId('.$cate.');';
//        $parse .= '$__LIST__ = M(\'Document\')->page(!empty($_GET["p"])?$_GET["p"]:1,'.$row.')->lists(';
		$parse .= '$__LIST__ = M(\'Doc0s1\')->page(!empty($_GET["p"])?$_GET["p"]:1,'.$row.')->lists(';
    ...
    public function _position($tag, $content){
        ...
        $name   = $tag['name'];
        $parse  = '<?php ';
        $parse .= '$__POSLIST__ = M(\'Document\')->position(';
        $parse .= $pos . ',';
        $parse .= $cate . ',';
        $parse .= $limit . ',';
        $parse .= $field . ');';
    ...
    public function _next($tag, $content){
        ...
        $info   = $tag['info'];
        $parse  = '<?php ';
        $parse .= '$' . $name . ' = M(\'Doc0s1\')->next($' . $info . ');'; //$parse .= '$' . $name . ' = M(\'Document\')->next($' . $info . ');';
        $parse .= ' ?>';
    ...
    public function _prev($tag, $content){
        ...
        $info   = $tag['info'];
        $parse  = '<?php ';
        $parse .= '$' . $name . ' = M(\'Doc0s1\')->prev($' . $info . ');'; //$parse .= '$' . $name . ' = M(\'Document\')->prev($' . $info . ');';
        $parse .= ' ?>';
        $parse .= '<notempty name="' . $name . '">';
        $parse .= $content;
        $parse .= '</notempty>';
    ...
    public function _partlist($tag, $content){
        ...
        $parse  = '<?php ';
        $parse .= '$__PARTLIST__ = M(\'Document\')->part(' . $id . ',  !empty($_GET["p"])?$_GET["p"]:1, \'' . $field . '\','. $listrow .');';
        $parse .= ' ?>';
