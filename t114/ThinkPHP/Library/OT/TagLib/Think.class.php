...
class Think extends TagLib{
    ...
    public function _nav($tag, $content){
        ...
        $parse  = $parse   = '<?php ';
        $parse .= '$__NAV__ = D(\'Channel\')->field('.$field.')->where("status=1")->order("sort")->select();';
        if($tree){
    ...
    public function _query($tag,$content) {
        ...
        $parseStr  =    '<?php $'.$result.' = D()->query("'.$sql.'");';
    ...
    public function _value($tag,$content,$type='getField'){
        ...
        $parseStr   =  '<?php echo '.$filter.'(D("'.$name.'")';
        if(!empty($tag['table'])) {
    ...
    public function _data($tag,$content){
        ...
        $parseStr   =   '<?php $'.$result.' =D("'.$name.'")->alias("__DOCUMENT")';
        if(!empty($tag['table'])) {
            $parseStr .= '->table("'.$tag['table'].'")';
        }
        if(!empty($tag['where'])){
    ...
    public function _datalist($tag,$content) {
        ...
        $parseStr   =   '<?php $_result = D("'.$name.'")->alias("__DOCUMENT")';
        if(!empty($tag['table'])) {
            $parseStr .= '->table("'.$tag['table'].'")';
        }
        if(!empty($tag['where'])){
    ...
    public function _cate($tag,$content){
        ...
        if(!empty($tag['id'])) {
            // 获取单个分类
            $parseStr   =  '<?php $'.$result.' = D("Category")->find('.$tag['id'].');';
            $parseStr .=  'if($'.$result.'):?>'.$content;
        }elseif(!empty($tag['name'])) {
            // 获取单个分类
            $parseStr   =  '<?php $'.$result.' = D("Category")->getByName('.$tag['name'].');';
            $parseStr .=  'if($'.$result.'):?>'.$content;
        }elseif(!empty($tag['pid'])){
            $key     =   !empty($tag['key'])?$tag['key']:'i';
            $mod    =   isset($tag['mod'])?$tag['mod']:'2';
            $parseStr   =  '<?php $_result = D("Category")->order("sort")->where("display=1 AND status=1 AND pid='.$tag['pid'].'")';
            if(!empty($tag['limit'])){
                $parseStr .= '->limit('.$tag['limit'].')';
            }
            $parseStr .= '->select();';
