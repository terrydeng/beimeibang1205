...
class IndexController extends Controller
{
    ...
    public function index($page = 1, $issue_id = 0)
    {
        ...
        $issue = M('Issue')->find($issue_id);
        if (!$issue_id == 0) {
            $issue_id = intval($issue_id);
            $issues = M('Issue')->where("id=%d OR pid=%d", array($issue_id, $issue_id))->limit(999)->select();
            $ids = array();
            foreach ($issues as $v) {
                $ids[] = $v['id'];
            }
        ...
        $map['status'] = 1;
        $content = M('IssueContent')->where($map)->order('create_time desc')->page($page, 16)->select();
        $totalCount = M('IssueContent')->where($map)->count();
        foreach ($content as &$v) {
            ...
            if($aDisplay_type=='masonry'){
                $cover = D('Picture')->where(array('status' => 1))->getById($v['cover_id']);
    ...
    public function doPost($id = 0, $cover_id = 0, $title = '', $content = '', $issue_id = 0, $url = '')
    {
        ...
        if ($id) {
            $content_temp = M('IssueContent')->find($id);
            ...
            if (modC('NEED_VERIFY', 0) && !is_administrator()) //需要审核且不是管理员
            {
                ...
                foreach ($admin_uids as $admin_uid) {
                    M('Common/Message')->sendMessage($admin_uid, $title = '专辑投稿提醒',"{$user['nickname']}向专辑投了一份稿件，请到后台审核。",  'Admin/Issue/verify', array(),is_login(), 2);
                }
            }
    ...
    public function issueContentDetail($id = 0)
    {
        ...
        $issue_content = M('IssueContent')->find($id);
        if (!$issue_content) {
            $this->error('404 not found');
        }
        M('IssueContent')->where(array('id' => $id))->setInc('view_count');
        $issue = M('Issue')->find($issue_content['issue_id']);
