...
class IssueController extends AdminController
{
    ...
    public function issue()
    {
        ...
        $tree = M('Issue/Issue')->getTree(0, 'id,title,sort,pid,status');
        $builder->title('专辑管理')
    ...
    {
        $effect_count = M('IssueContent')->where(array('issue_id' => $id))->setField('issue_id', $toid);
        M('Issue')->where(array('id' => $id))->setField('status', -1);
    ...
    public function setIssueContentStatus()
    {
        ...
        if ($status == 1) {
            foreach ($ids as $id) {
                $content = M('IssueContent')->find($id);
                M('Common/Message')->sendMessage($content['uid'],$title = '专辑内容审核通知', "管理员审核通过了您发布的内容。现在可以在列表看到该内容了。",  'Issue/Index/issueContentDetail', array('id' => $id), is_login(), 2);
                /*同步轻博客*/
