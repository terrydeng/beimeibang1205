...
class RechargeController extends AdminController
{
    ...
    public function alipayList($r = 15, $page = 1)
    {
        ...
        $recordModel = M('recharge_record_alipay');
    ...
    {
        $listBuilder = new AdminListBuilder();
        $recordModel = M('recharge_withdraw');
    ...
    public function doWithdraw($ids = array())
    {
        ...
        $withdrawModel = M('Recharge/Withdraw');
        foreach ($ids as $id) {
            ...
            //提现成功，向用户发送消息
            M("Common/Message")->sendMessageWithoutCheckSelf($withdraw['uid'], '您的提现已经受理，请注意查收。', '【充值中心】提现完成通知', U('recharge/index/withdrawList'), is_login());
        }
    ...
    public function cancelWithdraw($ids = array())
    {
        ...
        foreach ($ids as $id) {
            ...
            M('Ucenter/Score')->setUserScore($withdraw['uid'],  $withdraw['frozen_amount'], $withdraw['field'], 'inc','recharge_withdraw', $withdraw['id'],get_nickname( is_login()).'关闭提现订单');

            if (!$rs) {
                $withdrawModel->where(array('id' => $withdraw['id']))->setField('payok', 2); //待返还状态
                continue;
            }
            M("Common/Message")->sendMessageWithoutCheckSelf($withdraw['uid'],'【充值中心】提现关闭通知',  '您的提现已被关闭，冻结' . $withdraw['score_type']['title'] . $withdraw['frozen_amount'] . $withdraw['score_type']['unit'] . '已返还到您的账户。', 'recharge/index/withdrawList');
        }
        $this->success('关闭订单成功。');
