...
class IndexController extends Controller
{
    ...
    public function recharge()
    {
        ...
        if (IS_POST) {
            ...
            foreach ($fields as &$v) {
                $v['scoreType'] = M('Ucenter/Score')->getType(array('status' => 1, 'id' => $v['FIELD']));
    ...
    {
        $aOrderId = I('get.id', 0);
        $order = M('Order')->getOrder($aOrderId);
        if ($order['record_id']) {
            $record = M('Record')->getRecord($order['record_id'], $order['method']);
    ...
    private function createOrder()
    {
        ...
        $data['method'] = $aMethod;
        $data['uid'] = is_login();
        $order_id = M('Order')->addOrder($data);
        if ($order_id) {
            M('order_link')->add(array('order_id'=>$order_id,'model'=>'recharge_order','app'=>'recharge','method'=>$aMethod));
    ...
    private function createWithdraw()
    {
        ...
        $withdraw_id = M('Withdraw')->addWithdraw($data);

        M('Ucenter/Score')->setUserScore($loginUid, $forzen_count, $aField, 'dec','recharge_withdraw',$withdraw_id,get_nickname($loginUid).'提现');
    ...
    public function cancelWithdraw()
    {
        ...
        M('Ucenter/Score')->setUserScore($withdraw['uid'],  $withdraw['frozen_amount'], $withdraw['field'], 'inc','recharge_withdraw', $withdraw['id'],get_nickname( $withdraw['uid']).'取消提现订单');
