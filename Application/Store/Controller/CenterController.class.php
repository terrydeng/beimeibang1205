...
class CenterController extends BaseController
{
    ...
    public function _initialize()
    {
        ...
        $map['uid'] = is_login();
        $map['status'] = 1;
        $count['posted'] = M('Goods')->where($map)->count();

        $count['fav'] = M('store_fav')->where($map)->count();
        ...
        $this->shopModel=M('Store/StoreShop');
        parent::_initialize();
    }
    ...
    public function pay()
    {
        ...
        foreach ($goods['good_id'] as $v) {
            $goods['goods'][]['good'] = M('Info')->getById($v);
        }
    ...
    {
        $aOrderId = I('order_id', 0, 'floatval');
    ...
    {
        $aOrderId = I('id', 0, 'floatval');
        if ($aOrderId == 0) {
            $this->error('404');
        }

        $order = M('Order')->getById($aOrderId);
        if ($order['uid'] != is_login()) {
            ...
            $self['currency'] = M('Currency')->getCurrency(is_login());
    ...
    public function response()
    {
        ...
        $responses = M('store_order')->where($map)->order('response_time desc')->findPage(10);
    ...
    public function send()
    {
        ...
        if ($rs) {
            $this->messageModel->sendMessage($order['uid'], $content = '【微店】订单.' . $order['id'] . '商家已发货！', $title = '微店订单发货通知', 'store/center/orders',array(), is_login());
            $this->success('发货成功。');
        } else {
    ...
    {
        M('Cart')->setItemCount($_POST['good_id'], $_POST['count']);
        exit(json_encode(array('status' => 1)));
    }
    ...
    public function fav()
    {
        ...
        //获取该用户发布的全部用户组
        $t_map['uid'] = get_uid();
        $list = M('store_fav')->where($t_map)->findPage(4);
        $goodsModel = M('Goods');
    ...
    public function createShop()
    {
        ...
        if (IS_POST) {
            ...
            $this->assign('top_tab','createshop');
            $this->display();
        }
    }
    ...
    public function post()
    {
        ...
        $my_shop = $this->shopModel->getShopByUid($uid);
        ...
        $this->assign('cats', M('Store/Category')->getCatAll());
        if (is_null($my_shop)) {
        ...
        if ($entity['name'] == 'good') {
            if ($rs_info && $rs_data) {
                ...
                if ($entity['need_active']) {
                    ...
                    $this->success('发布商品成功。即将跳转到商品页面。点击<a href="' . U('store/center/post', array('entity_id' => $entity['id'])) . '">继续发布</a>。', U('store/Index/info', array('info_id' => $rs_info)), 5);
                }
            } else {
