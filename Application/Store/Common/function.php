...
{
    $myshop = M('Store/StoreShop')->getShopByUid($uid);
    $shop_id = $myshop['id'];
    return M('Goods')->where('shop_id=' . $shop_id)->count();
}

function checkHasCreatedShop($uid)
{
    $my_shop = M('Store/StoreShop')->getShopByUid($uid);
    return $my_shop;
}
...
function CheckCanPostEntityN($uid, $entity_name)
{
    ...
    $entity = M('store_entity')->where($map)->limit(1)->select();
...
function CheckCan($uid, $entity_id, $can_type)
{
    if (is_administrator())
        return true;
    $entity = M('store_entity')->find($entity_id);
...
function parse_price($str){
    ...
    $l = cut_str('.',$str,'r');
    $r = cut_str('.',$str,'l');
    return $l.'<span style="font-size:18px;">.'.$r.'</span>';
}