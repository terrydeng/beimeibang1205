...
class OrderModel extends Model
{
    ...
    function addOrder($order, $good_ids, $good_counts)
    {
        ...
        foreach ($goods as $key => $v) {
            ...
            $shop = M('Store/StoreShop')->getById($v[0]['shop_id']);
            $rs = $this->add($t_order);
            if ($rs == 0) {
                return array(false, $this->getError());
            }

            M('order_link')->add(array('order_id'=>$rs,'model'=>'store_order','app'=>'store'));
            //给店主发信息
            M('Message')->sendMessage($shop['uid'], $content = '【微店】订单' . $rs . '已创建，等待买家付款！', $title = '微店下单通知', 'store/center/sold',array(), is_login());
            ...
            foreach ($v as $k => $g) {
                ...
                $rs_good = M('store_item')->add($t_good);
                M('Goods')->where('id=' . $g['id'])->setField('sell', $g['sell'] + $g['count']);
                if ($rs_good == 0) {
                    return array(false, $this->getError());
                }
            }
    ...
    function comput_sum($items)
    {
        ...
        foreach ($items as $k_item => $v_item) {
            $orders['data'][$k_item]['goods'][] = M('Info')->getById($v_item['good_id']);
    ...
    function pay($order_id)
    {
        ...
        if ($currencyModel->pay($cost, is_login())) {
            ...
            M('Ucenter/Score')->addScoreLog(is_login(),  $cost, modC('CURRENCY_TYPE', '4', 'Store'), 'dec','recharge_order',$order_id,get_nickname( is_login()).'在微店支付了订单');
            ...
            if ($this->save($order)) {
                ...
                //给店主发信息
                M('Message')->sendMessage($shop['uid'], $content = '【微店】订单' . $order['id'] . '已付款，赶紧发货吧！', $title = '微店订单付款通知', 'store/center/sold',array(), is_login());
                return true;
            } else {
        ...
        if ($order) {
            ...
            if ($r) {
                ...
                if ($rs) {
                    ...
                    M('Ucenter/Score')->addScoreLog($order['s_uid'],  getFinalPrice($order), modC('CURRENCY_TYPE', '4', 'Store'), 'inc','recharge_order',$order_id,get_nickname($order['s_uid']).'收到了'.get_nickname(is_login()).'付的款');

                    $shopModel = M('Store/StoreShop');
                    $shop = $shopModel->where(array('uid' => $order['s_uid']))->find();
                    M('Message')->sendMessage($shop['uid'], $content = '【微店】订单' . $order['id'] . '买家已确认收货，赶紧查查款项吧！', $title = '微店订单确认收货通知', 'store/center/sold',array(), is_login());
                    return $rs;
                } else {
    ...
    function response($order_id, $response, $content)
    {
        ...
        if ($rs) {
            $shop = $this->getShopFromOrder($order);
            M('Message')->sendMessage($shop['uid'], $content = '【微店】买家已修改评价' . $order['id'] . '！', $title = '微店订单评价通知', 'store/center/sold',array(), is_login());
            return true;
        } else {
            $this->error = '数据更改错误。';
            return false;
    ...
    function getList($map = array(), $num = 10, $order = 'create_time desc')
    {
        ...
        foreach ($orders['data'] as $k => $v) {
            ...
            $map_i['order_id'] = $v['id'];
            $items = M('store_item')->where($map_i)->select();
            $orders['data'][$k]['items'] = $items;
            foreach ($items as $k_item => $v_item) {
                $item = array('good' => M('Goods')->getById($v_item['good_id']), 'item' => $v_item);
    ...
    function getById($order_id)
    {
        ...
        $order['items'] = M('store_item')->where($map_item)->limit(999)->select();
        foreach ($order['items'] as $k => $v) {
            $order['items'][$k]['good'] = M('Info')->getById($v['good_id']);
        }
        return $order;
    }
