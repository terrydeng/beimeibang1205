...
class CategoryModel extends Model
{
    ...
    public function getAllChildrenCats($pid = 0,$limit=0)
    {
        if($limit){
            $cats = $this->where(array('pid' => $pid,'status'=>1))->order('sort desc')->limit($limit)->select();
        }else{
            $cats = $this->where(array('pid' => $pid,'status'=>1))->order('sort desc')->select();
        }
     ...
     */
    public function getSelectTop($tree)
    {
        $parentSelect = array(0=>'-顶级分类-');
        foreach ($tree as $v) {
            $parentSelect[$v['id']] = $v['title'];
        }
        return $parentSelect;
    }
