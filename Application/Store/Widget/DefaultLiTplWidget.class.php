    ...
    public function render($data)
    {
        ...
        if ($data['entity']['name'] == 'shop') {
            ...
            foreach ($data['infos']['data'] as $key => $v) {
                $data['infos']['data'][$key]['goods'] = M('Info')->getLimit("shop_id=" . $v['id'].' and status=1', 8, 'create_time desc');
                foreach ($data['infos']['data'][$key]['goods'] as $k => $vo) {
                    $data['infos']['data'][$key]['goods'][$k]['data'] = M('Data')->getByInfoId($vo['id']);
                }
            }
            $this->assign($data);
            $this->display('Widget/DefaultLiTpl/shop');
                ...
                foreach ($data['infos'] as $key => $v) {
                    $data['infos'][$key]['goods'] = M('Info')->getLimit("shop_id=" . $v['id'].' and status=1', 12, 'create_time desc');
                    foreach ($data['infos'][$key]['goods'] as $k => $vo) {
                        $data['infos'][$key]['goods'][$k]['data'] = M('Data')->getByInfoId($vo['id']);
                    }
                }
                $this->assign($data);
                $this->display('Widget/DefaultLiTpl/shop');
        ...
        } else if($data['entity']['name']=='good')
        {
            ...
            {
                foreach ($data['infos'] as $key => $v) {
                    $data['infos'][$key]['shop'] = M('Info')->getById($v['shop_id']);
                }
            }else{
                foreach ($data['infos']['data'] as $key => $v) {
                    $data['infos']['data'][$key]['shop'] = M('Info')->getById($v['shop_id']);
                }
            }
            $this->assign($data);
