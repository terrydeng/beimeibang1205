...
class InfoListWidget extends Controller
{
    ...
    public function render($var)
    {
        ...
        if (isset($var['map']['type'])) {
            $cateLevel = M('Store/Category')->getLevel($var['map']['type']) - 1;
        ...
        if (isset($var['map']) && count($var['map'])) {
            if (isset($map['entity_id'])) {
                ...
                foreach ($map_params as $param_name => $param_value) {
                    ...
                    $search_field = M('store_field')->where($field_map)->find();
                    ...
                    //进行查询
                    $store_data = M('store_data')->where($data_map)->select();
        ...
        if ($this->_type == 'list') {
            $infos = M('Goods')->getList($map, $num, $order);
            $info_count = count($infos['data']);
        } else {
            $infos = M('Goods')->getLimit($map, $num, $order);
    ...
    private function fetchTpl($tpl, $infos, $entity, $info_count)
    {
        ...
        switch ($tpl) {
            ...
            case 0: //解析预置模板
                ...
                $tpl_html = M('Render')->renderInfoLi($tpl_html, $infos, $this->_class, $this->_type);
                break;
    ...
    private function getEntity($data)
    {
        if (intval($data['entity_id']) != 0) {
            //获取预置的模板
            $entity = M('store_entity')->find(intval($data['entity_id']));
            return $entity;
        } else {
            //通过name查到entity,和entity的id
            $map_t['name'] = $data['name'];
            $entity = M('store_entity')->where($map_t)->find();
            return $entity;
        }
    }
}