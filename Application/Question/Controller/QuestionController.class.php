...
class QuestionController extends AdminController
{
    ...
    {
        parent::_initialize();
        $this->questionModel = M('Question/Question');
        $this->questionAnswerModel = M('Question/QuestionAnswer');
        $this->questionCategoryModel = M('Question/QuestionCategory');
        $this->questionSupportModel = M('Question/QuestionSupport');
    }
    ...
    public function config()
    {
        ...
        $builder->keyRadio('QUESTION_NEED_AUDIT', '问题是否需要审核', '', array('0' => '否', '1' => '是'))->keyDefault('QUESTION_NEED_AUDIT', 1)
            ->keyInteger('QUESTION_ANSWER_MIN_NUM','问题回答最少字数限制')->keyDefault('QUESTION_ANSWER_MIN_NUM',20)
            ...
            ->group('基本配置', 'QUESTION_NEED_AUDIT,QUESTION_ANSWER_MIN_NUM')->group('首页展示配置', 'QUESTION_SHOW_TITLE,QUESTION_SHOW_COUNT,QUESTION_SHOW_TYPE,QUESTION_SHOW_ORDER_FIELD,QUESTION_SHOW_ORDER_TYPE,QUESTION_SHOW_CACHE_TIME')
        ...
        if($res){
            if($recommend==1){
                ...
                foreach($ids as $val){
                    ...
                    $messageModel->sendMessage($question['uid'], '问题被推荐', '你的问题【'.$question['title'].'】被管理员设为推荐！', 'Question/Index/detail',array('id'=>$val), is_login(), 0);
                }
            }
            $this->success('操作成功！');
        }else{
        ...
        if($status==1){
            foreach($ids as $val){
                ...
                if($question['status']==2){
                    $messageModel->sendMessage($question['uid'], '问题被审核', '你的问题【'.$question['title'].'】通过了审核！！', 'Question/Index/detail',array('id'=>$val), is_login(), 2);
                }
            }
        }else if($status==0){
            foreach($ids as $val){
                ...
                $question=$this->questionModel->find($val);
                if($question['status']==2){
                    $messageModel->sendMessage($question['uid'], '问题审核失败', '你的问题【'.$question['title'].'】没有通过审核！！', 'Question/Index/detail',array('id'=>$val), is_login(), 2);
                }
            }
        }else{
            foreach($ids as $val){
                ...
                $question=$this->questionModel->find($val);
                if($question['status']==2){
                    $messageModel->sendMessage($question['uid'], '问题审核失败', '你的问题【'.$question['title'].'】被管理员直接删除！！', 'Question/Index/myQuestion', is_login(), 2);
                }
            }
        }
