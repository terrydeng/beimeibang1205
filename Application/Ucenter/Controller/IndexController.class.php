...
class IndexController extends BaseController
{
    ...
    public function information($uid = null)
    {
        ...
        if ($user['pos_province'] != 0) {
            $user['pos_province'] = M('district')->where(array('id' => $user['pos_province']))->getField('name');
            $user['pos_city'] = M('district')->where(array('id' => $user['pos_city']))->getField('name');
            $user['pos_district'] = M('district')->where(array('id' => $user['pos_district']))->getField('name');
            $user['pos_community'] = M('district')->where(array('id' => $user['pos_community']))->getField('name');
        }
        //显示页面
    ...
    public function _fans_and_following($uid = null)
    {
        ...
        //我的粉丝展示
        $map['follow_who'] = $uid;
        $fans_default = M('Follow')->where($map)->field('who_follow')->order('create_time desc')->limit(8)->select();
        $fans_totalCount = M('Follow')->where($map)->count();
        ...
        $follow_default = M('Follow')->where($map_follow)->field('follow_who')->order('create_time desc')->limit(8)->select();
        $follow_totalCount = M('Follow')->where($map_follow)->count();
    ...
    public function fans($uid = null, $page = 1)
    {
        ...
        $fans = M('Follow')->getFans($uid, $page, array('avatar128', 'uid', 'nickname', 'fans', 'following', 'weibocount', 'space_url', 'title'), $totalCount);
    ...
    public function following($uid = null, $page = 1)
    {
        ...
        $following = M('Follow')->getFollowing($uid, $page, array('avatar128', 'uid', 'nickname', 'fans', 'following', 'weibocount', 'space_url', 'title'), $totalCount);
    ...
    public function rank($uid = null)
    {
        ...
        $rankList = M('rank_user')->where(array('uid' => $uid, 'status' => 1))->field('rank_id,reason,create_time')->select();
        foreach ($rankList as &$val) {
            $rank = M('rank')->where('id=' . $val['rank_id'])->find();
            $val['title'] = $rank['title'];
            $val['logo_url'] = get_pic_src(D('picture')->where('id=' . $rank['logo'])->field('path')->getField('path'));
    ...
    public function rankVerifyFailure()
    {
        ...
        $rankList = M('rank_user')->where(array('uid' => $uid, 'status' => -1))->field('id,rank_id,reason,create_time')->select();
        foreach ($rankList as &$val) {
            $rank = M('rank')->where('id=' . $val['rank_id'])->find();
            $val['title'] = $rank['title'];
            $val['logo_url'] = get_pic_src(D('picture')->where('id=' . $rank['logo'])->field('path')->getField('path'));
    ...
    public function rankVerifyWait()
    {
        ...
        $rankList = M('rank_user')->where(array('uid' => $uid, 'status' => 0))->field('rank_id,reason,create_time')->select();
        foreach ($rankList as &$val) {
            $rank = M('rank')->where('id=' . $val['rank_id'])->find();
            $val['title'] = $rank['title'];
            $val['logo_url'] = get_pic_src(D('picture')->where('id=' . $rank['logo'])->field('path')->getField('path'));
    ...
    public function rankVerifyCancel($rank_id = null)
    {
        ...
        if (is_login() && $rank_id) {
            ...
            if ($result) {
                M('Message')->sendMessageWithoutCheckSelf(is_login(),L('_MESSAGE_RANK_CANCEL_1_'),  L('_MESSAGE_RANK_CANCEL_2_'), 'Ucenter/Message/message', array('tab' => 'system'));
    ...
    public function rankVerify($rank_user_id = null)
    {
        ...
        if ($rank_user_id) {
            ...
            M('Message')->sendMessageWithoutCheckSelf(is_login(), L(''),L(''),  'Ucenter/Message/message', array('tab' => 'system'));
        }
        $alreadyRank = M('rank_user')->where($map_already)->field('rank_id')->select();
        $alreadyRank = array_column($alreadyRank, 'rank_id');
        if ($alreadyRank) {
            $map['id'] = array('not in', $alreadyRank);
        }
        $map['types'] = 1;
        $rankList = M('rank')->where($map)->select();
        foreach($rankList as &$rank){
            $rank['logo_url'] = get_pic_src(D('picture')->where('id=' . $rank['logo'])->field('path')->getField('path'));
        }
        unset($rank);
        $this->assign('rankList', $rankList);
    ...
    public function verify($rank_id = null, $reason = null, $rank_user_id = 0)
    {
        ...
        if ($result) {
            M('Message')->sendMessageWithoutCheckSelf(is_login(),L('_MESSAGE_RANK_APPLY_1_'),L('_MESSAGE_RANK_APPLY_2_'),  'Ucenter/Message/message', array('tab' => 'system'));
