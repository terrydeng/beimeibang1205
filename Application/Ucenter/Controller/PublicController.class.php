...
class PublicController extends Controller
{
    ...
    private function getProfile($uid)
    {
        ...
        $userProfile['followed'] = M('Follow')->where($follow)->count();
    ...
    public function card()
    {
        ...
        $follow=M('Common/Follow')->isFollow(is_login(),$aUID);
    ...
    public function getInformation()
    {
        ...
        $new_talks = M('TalkPush')->getAllPush(); //聊天推送
        M('TalkPush')->where(array('uid' => get_uid(), 'status' => 0))->setField('status', 1); //读取到推送之后，自动删除此推送来防止反复推送。

        $new_talk_messages = M('TalkMessagePush')->getAllPush(); //聊天消息推送
        M('TalkMessagePush')->where(array('uid' => get_uid(), 'status' => 0))->setField('status', 1); //读取到推送之后，自动删除此推送来防止反复推送。

        foreach ($new_talk_messages as &$message) {
            $message = M('TalkMessage')->find($message['source_id']);
    ...
    {
        M('Message')->setAllReaded(is_login());
    }
    ...
    {
        exit(json_encode(array('status' => M('Common/Message')->readMessage($message_id))));
