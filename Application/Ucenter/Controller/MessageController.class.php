...
class MessageController extends BaseController
{
    ...
    public function message($page = 1, $tab = 'unread')
    {
        ...
        $messages = M('Message')->where($map)->order('create_time desc')->page($page, 10)->select();
        $totalCount = M('Message')->where($map)->order('create_time desc')->count(); //用于分页

        foreach ($messages as &$v) {
            $v['content'] = M('Common/Message')->getContent($v['content_id']);
            if ($v['from_uid'] != 0) {
    ...
    public function talk($message_id = 0, $talk_id = 0)
    {
        ...
        $map['talk_id'] = $talk['id'];
        $messages = M('TalkMessage')->where($map)->order('create_time desc')->limit(20)->select();
    ...
    public function doDeleteTalk($talk_id = 0)
    {
        ...
        if (count($uids) <= 2) {
            M('Talk')->where(array('id' => $talk_id))->setField('status', -1);
            M('Message')->where(array('talk_id' => $talk_id))->setField('talk_id', 0);
        } //如果删除前聊天中有多个人，就退出聊天。
        else {
            $uids = array_diff($uids, array("[$uid]"));
            $uids = implode(',', $uids);
            M('Talk')->where(array('id' => $talk_id))->save(array('uids' => $uids));
            M('Message')->where(array('talk_id' => $talk_id, 'uid' => get_uid()))->setField('talk_id', 0);
        }
    ...
    private function getMessageModel($message)
    {
        ...
        $messageModel = M($appname . '/' . $appname . 'Message');
        return $messageModel;
    }
    ...
    private function getTalk($message_id, $talk_id)
    {
        if ($message_id != 0) {
            ...
            $message = M('Message')->find($message_id);
            ...
            //创建聊天
            $talk = M('Talk')->create($talk);
            $talk['id'] = M('Talk')->add($talk);
            /*创建talk end*/
            ...
            $talkMessageModel = M('TalkMessage');
            $talkMessage = $talkMessageModel->create($talkMessage);
            $talkMessage['id'] = $talkMessageModel->add($talkMessage);


            M('Message')->sendMessage($message['from_uid'],L('_MESSAGE_CHAT_1_'),  L('_MESSAGE_CHAT_2_') . $talk['title'], 'Ucenter/Message/talk', array('talk_id' => $talk['id']));
