...
class InviteController extends BaseController
{
    ...
    {
        parent::_initialize();
        $this->mInviteModel=M('Ucenter/Invite');
        $this->mInviteTypeModel=M('Ucenter/InviteType');
        $this->mInviteBuyLogModel=M('Ucenter/InviteBuyLog');
        $this->mInviteUserInfoModel=M('Ucenter/InviteUserInfo');
    }
    ...
    public function exchange()
    {
        if(IS_POST){
            ...
            M('Ucenter/Score')->setUserScore(array(is_login()),$aNum*$inviteType['pay_score'],$inviteType['pay_score_type'],'dec','',0,L('_INV_QUOTA_2_'));//扣积分
    ...
    private function _checkCanBuy($inviteType=0,$num=0)
    {
        ...
        if($inviteType['auth_groups']!=''){
            ...
            $map['uid']=is_login();
            if(!M('AuthGroupAccess')->where($map)->count()){
