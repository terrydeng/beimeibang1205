...
class CollectionController extends BaseController{
    ...
    public function _getList($type='forum',&$totalCount=0,$page=1,$r=15)
    {
        ...
        switch ($type) {
            case 'forum':
                ...
                $post_ids=M('ForumBookmark')->where($map)->field('post_id')->select();
            ...
            case 'group':
                ...
                $post_ids=M('GroupBookmark')->where($map)->field('post_id')->select();
                $post_ids=array_column($post_ids,'post_id');
    ...
    private function getForumList()
    {
        ...
        if (empty($forum_list)) {
            //读取板块列表
            $forum_list = M('Forum/Forum')->where(array('status' => 1))->order('sort asc')->select();
    ...
    private function getGroupList()
    {
        ...
        if (empty($group_list)) {
            //读取板块列表
            $group_list = M('Group/Group')->where(array('status' => 1))->order('sort asc')->select();
