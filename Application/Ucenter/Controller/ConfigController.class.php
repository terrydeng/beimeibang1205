...
1600:Lv7 '.L('_CHAIRMAN__').'
        ...
        $action = M('Admin/Action')->getAction(array('status' => 1));
        $action_module = array();
        foreach ($action as &$v) {
            ...
            foreach ($v['rule_array'] as &$o) {
                ...
                $o['score'] = M('Score')->getType(array('id' => $o['field']));
            }
        ...
        foreach ($action_module as $key => &$a) {
            ...
            $a['module'] = M('Common/Module')->getModule($key);
        }
    ...
    public function role()
    {
        ...
        if (IS_POST) {
            ...
            $result = M('Member')->where(array('uid' => is_login()))->setField('show_role', $aShowRole);
            if ($result) {
    ...
    public function tag()
    {
        $userTagLinkModel = M('Ucenter/UserTagLink');
        if (IS_POST) {
            ...
            $ids = D('RoleConfig')->where($map)->getField('value');
            if ($ids) {
                $ids = explode(',', $ids);
    ...
    private function checkNickname($nickname)
    {
        ...
        $had_nickname = M('Member')->where($map_nickname)->count();
        if ($had_nickname) {
            $this->error(L('_ERROR_NICKNAME_USED_').L('_PERIOD_'));
        }
        $denyName = D("Config")->where(array('name' => 'USER_NAME_BAOLIU'))->getField('value');
        if ($denyName != '') {
    ...
    {
        $res = M('field_group')->where(array('id' => $profile_group_id, 'status' => '1'))->find();
        if (!$res) {
            return array();
        }
    ...
    public function checkVerify()
    {
        ...
        $res = M('Verify')->checkVerify($aAccount, $aType, $aVerify, $aUid);
        if (!$res) {
    ...
    public function cleanRemember()
    {
        $uid = is_login();
        if ($uid) {
            M('user_token')->where('uid=' . $uid)->delete();
