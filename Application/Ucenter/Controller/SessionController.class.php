...
class SessionController extends BaseController
{
    ...
    public function session()
    {
        ...
        foreach ($talks as $key => $v) {
            ...
            $talks[$key]['last_message'] = M('Talk')->getLastMessage($talks[$key]['id']);
        }
    ...
    public function doDeleteTalk($talk_id)
    {
        ...
        if (count($uids) <= 2) {
            ...
            D('talk_message_push')->where(array('talk_id' => $talk_id))->delete();
            D('talk_push')->where(array('source_id' => $talk_id))->delete();
    ...
    public function postMessage($content, $talk_id)
    {
        ...
        M('TalkMessage')->sendMessage($content, $this->mTalkModel->getUids($talk['uids']), $talk_id);
        if (!$rs) {
