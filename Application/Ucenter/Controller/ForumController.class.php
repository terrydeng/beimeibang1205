...
class ForumController extends BaseController {
    ...
    public function myTakePartIn($page=1,$uid=0) {
        ...
        //依次读取帖子详情
        foreach($list as &$e) {
            $e = M('Forum/ForumPost')->where(array('id'=>$e['post_id']))->find();
        }
    ...
    public function myBookmark($page=1) {
        ...
        //读取数据库获取已经收藏的帖子
        $map = array('uid'=>$uid);
        $model = M('ForumBookmark');
        $list = $model->where($map)->order('create_time desc')->page($page)->select();
        $totalCount = $model->where($map)->count();

        //读取帖子的详情
        foreach($list as &$e) {
            $e = M('ForumPost')->where(array('id'=>$e['post_id']))->find();
        }
