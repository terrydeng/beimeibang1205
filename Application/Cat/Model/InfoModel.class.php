...
class InfoModel extends Model implements IBaseModel
{
    ...
    {
        $rs = M('cat_info')->where($map)->order($order)->findPage($num);
        foreach ($rs['data'] as $key => $v) {
            $rs['data'][$key]['data'] = M('Data')->getByInfoId($v['info_id']);
        }
    ...
    function checkOwner($mid, $info_id)
    {
        $map['uid'] = $mid;
        $map['info_id'] = $info_id;
        $is = M('cat_info')->where($map)->count();
        return $is;
    }

    function getLimit($map = '', $num = 10, $order = 'create_time desc')
    {
        $rs = M('cat_info')->where($map)->order($order)->limit($num)->select();
        foreach ($rs as $key => $v) {
            $rs[$key]['data'] = M('Data')->getByInfoId($v['info_id']);
        }
    ...
    function getById($id)
    {
        $map['id'] = $id;
        $info = $this->find($id);
        $info['data'] = M('Data')->getByInfoId($id);
        return $info;
    }
