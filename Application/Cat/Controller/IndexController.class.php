...
class IndexController extends BaseController
{
    ...
    public function index()
    {
        ...
        $map['show_index'] = '1';
        $map['status'] = 1;
        $entitys = M('cat_entity')->where($map)->order('sort desc')->select();
    ...
    public function post()
    {
        ...
        if (I('get.entity_id', 0, 'intval') != 0) {
            $entity = M('cat_entity')->find(I('get.entity_id', 0, 'intval'));
        }
    ...
    public function info()
    {
        ...
        //获取到信息的数据
        $info['data'] = M('Data')->getByInfoId($info['id']);
        /*得到实体信息end*/
        $tpl = '';
        /*构建自动生成模板*/
        ...
        if ($entity['use_detail'] == 0) {
            ...
            /**默认模板添加**/
            $assign['entity'] = M('cat_entity')->find($info['entity_id']);
            $assign['data'] = M('Data')->getByInfoId($info['id']);
            $assign['user'] = query_user(array('uid','nickname', 'spcae_url'), $info['uid']);
    ...
    {
        $infos = M('Info')->getList('entity_id=' . I('get.entity_id') . ' and uid=' . is_login() . ' and status=1');
