...
class CatController extends AdminController
{
    ...
    public function setFieldStatus($ids, $status)
    {
        ...
        M('cat_field')->where(array('id' => $field_id))->setField('status', $status);
        M('cat_data')->where(array('field_id' => $field_id))->setField('status', $status);
        $this->success('操作成功。');
    }
    ...
    public function editField()
    {
        ...
        if (IS_POST) {
            ...
            $field = M('cat_field')->create();
            $field['status'] = 1;


            $rs = M('cat_field')->add($field, null, true);
            if ($rs) {
                $this->success('修改成功。');
            } else {
                $this->error('修改失败。');
    ...
    {
        M('cat_entity')->where(array('id' => array('in', implode(',', $ids))))->setField('status', $status);
        //删除关联信息
        M('cat_info')->where(array('entity_id' => array('in', implode(',', $ids))))->setField('status', $status);
        $this->success('操作成功。');
    }
    ...
    public function editEntity($entity_id = 0)
    {
        ...
        if (IS_POST) {
            ...
            $entitys = M('cat_entity')->where(array('status'=>array('gt',-1)))->limit(999)->select();
