...
function send_mob_weibo($content,$type,$feed_data = '', $from=''){
    ...
    $uid = is_login();
    M('Topic')->addTopic($content);
    $weibo_id = M('Weibo')->addWeibo($uid, $content, $type, $feed_data,$from);
    if (!$weibo_id) {
        return false;
    }
...
/*function send_comment_message($uid, $id, $message,$pid){
 ...
 //   $title = '评论消息';
    ...
    switch($pid){
        case 'weibo':
            $title = '评论消息';
            M('Common/Message')->sendMessage($uid,$title, $message,  'Weibo/Index/weiboDetail',array('id' => $id), $from_uid, $type);
            break;
        case 'issue':
            if($uid==is_login()){
                break;
            ...
            M('Common/Message')->sendMessage($uid,$title, $message,  'Issue/Index/IssueContentDetail',array('id' => $id), $from_uid, $type);
            break;
        case 'blog':
            if($uid==is_login()){
                break;
            ...
            $title =get_nickname($from_uid). '评论了您';
            M('Common/Message')->sendMessage($uid,$title, $message,  'news/index/detail',array('id' => $id), $from_uid, $type);
            break;
    }
...
function send_at_message($uids, $weibo_id, $content)
{
    ...
    foreach ($uids as $uid) {
        ...
        $fromUid = get_uid();
        $messageType = 1;
        M('Common/Message')->sendMessage($uid, $title, $message, 'Weibo/Index/weiboDetail',array('id' => $weibo_id), $fromUid, $messageType);
    }
}
...
function parse_topic($content){
    ...
    //将##替换成链接
    foreach ($topic as $e) {
        $tik = M('Weibo/Topic')->where(array('name' => $e))->find();

        //没有这个话题的时候创建这个话题
        if($tik){
            //M('Weibo/Topic')->add(array('name'=> $e));
...
function group_is_exist($group_id){
    $group =  M('Group/Group')->getGroup($group_id);
    return $group ? true : false;
}

function get_group_admin($group_id){
    return M('Group/GroupMember')->getGroupAdmin($group_id);
}

//MOB模块得到用户头衔
function mob_get_head_title($user_id){
    $user_title = M('RankUser')->where(array('status' => 1, 'uid' => $user_id))->select();
...
function parse_at_mob_users($content,$disabel_hight=false)
{
    $content = $content . ' ';
    //找出被AT的用户
    $at_usernames = get_at_users($content); //$at_usernames = "adm1n";  //$at_usernames = get_at_usernames($content);
    //将@用户替换成链接
...
function limit_picture_count($content){
    return   M('ContentHandler')->limitPicture($content,modC('GROUP_POST_IMG_COUNT',10,'GROUP'));
}

function get_group_name($group_id){
    $group =  M('Group')->getGroup($group_id);
    return $group['title'];
}
//群组
function is_joined($group_id)
{
    return M('Group/GroupMember')->getIsJoin(is_login(),$group_id);
}
...
function get_admin_ids($id=0,$type=0,$with_self=1)
{
    ...
    switch($type){
        case '1'://根据贴子楼中楼回复id查询排除者id
            $lzl_reply=D('GroupLzlReply')->find($id);
            $uid=$lzl_reply['uid'];
            $post_id=$lzl_reply['post_id'];
            break;
        case '2'://根据贴子回复id查询排除者id
            $reply = D('GroupPostReply')->find($id);
            $uid=$reply['uid'];
    ...
    $expect_ids=M('GroupMember')->getGroupAdmin($group_id);
    $group=D('Group')->find($group_id);
...
function CheckCan($uid, $entity_id, $can_type)
{
    ...
    $group_result = M('auth_group_access')->field('group_id')->where(array('uid' => $uid))->select();
