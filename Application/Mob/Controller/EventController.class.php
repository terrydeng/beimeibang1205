...
class EventController extends BaseController
{
    ...
    {
        $this->eventModel = M('Event/Event');
        $this->eventAttendModel = M('Event/EventAttend');
        $this->eventTypeModel = M('Event/EventType');
        ...
        $tree = M('EventType')->where(array('status' => 1))->select();
    ...
    public function member($id = 0, $tip = 'all')
    {
        ...
        foreach ($member as &$v) {
            ...
            $v['attach_info'] = M('File')->find($v['attach']);
    ...
    public function doSign($event_id, $name, $phone)
    {
        ...
        if (!$check) {
            ...
            if ($res) {
                M('Message')->sendMessageWithoutCheckSelf($event_content['uid'], '报名通知', query_user('nickname', is_login()) . '报名参加了活动]' . $event_content['title'] . ']，请速去审核！', 'Event/Index/member', array('id' => $event_id));
    ...
    public function shenhe($uid, $event_id, $tip)
    {
        ...
        if ($tip) {
            ...
            M('Message')->sendMessageWithoutCheckSelf($uid, '审核通知', query_user('nickname', is_login()) . '已经通过了您对活动[' . $event_content['title'] . ']的报名请求', 'Event/Index/detail', array('id' => $event_id));
        } else {
            $this->eventModel->where(array('id' => $event_id))->setDec('attentionCount');
            M('Message')->sendMessageWithoutCheckSelf($uid, '取消审核通知', query_user('nickname', is_login()) . '取消了您对活动[' . $event_content['title'] . ']的报名请求', 'Event/Index/member', array('id' => $event_id));
        }
        if ($res) {
            $this->success('操作成功');
        } else {
    ...
    public function unSign($event_id)
    {
        ...
        if ($res) {
            ...
            M('Message')->sendMessageWithoutCheckSelf($event_content['uid'], '取消报名通知', query_user('nickname', is_login()) . '取消了对活动[' . $event_content['title'] . ']的报名', 'Event/Index/detail', array('id' => $event_id));
