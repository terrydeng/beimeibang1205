...
class QuestionController extends BaseController
{
    ...
    public function index($mark = 0,$typeId=0)      //,待回答mark=>0，热门问题=>1，我的问题=>2，全部问题=>3
    {
        ...
        switch ($mark) {
            case '0':
                ...
                $questionlist = M('Question')->where(array('status' => 1, 'best_answer' => 0))->order('create_time desc')->page($aPage, $aCount)->select();
                $totalCount = M('Question')->where(array('status' => 1, 'bast_answer' => ""))->count();

                break;
            case '1':
                $this->setTopTitle('热门问题');
                $questionlist = M('Question')->where(array('status' => 1))->order('is_recommend desc,create_time asc,answer_num desc')->page($aPage, $aCount)->select();
                $totalCount = M('Question')->where(array('status' => 1))->count();
                break;
            case '2':
                $this->setTopTitle('我的问答');
                $questionlist = M('Question')->where(array('status' => 1, 'uid' => is_login()))->order('create_time desc')->page($aPage, $aCount)->select();
                $totalCount = M('Question')->where(array('status' => 1, 'uid' => is_login()))->count();
                break;
            case '3':
                $this->setTopTitle('全部问答');
                $questionlist = M('Question')->where(array('status' => 1))->order('create_time desc')->page($aPage, $aCount)->select();
                $totalCount = M('Question')->where(array('status' => 1))->count();
                break;
            case 'parentType':
                $this->assign('typeId', $typeId);
                $typeName['parent']=M('QuestionCategory')->where(array('id'=>$typeId))->find();

                $typeName['child']=M('QuestionCategory')->where(array('pid'=>$typeName['parent']['id']))->select();
                if(is_null( $typeName['child'])){
                    $this->setTopTitle($typeName['parent']['title']);
                    $questionlist = M('Question')->where(array('category'=>$typeId,'status' => 1))->order('create_time desc')->page($aPage, $aCount)->select();
                    $totalCount = M('Question')->where(array('category'=>$typeId,'status' => 1))->count();
                }else{
                    ...
                    $questionlist = M('Question')->where($map,array('status' => 1))->order('create_time desc')->page($aPage, $aCount)->select();
                    $totalCount = M('Question')->where($map,array('status' => 1))->count();
                }

                break;
            case 'childType':
                $this->assign('typeId', $typeId);
                $typeName=M('QuestionCategory')->where(array('id'=>$typeId))->find();
        ...
        foreach ($questionlist as &$v) {
            ...
            $v['category'] = M('QuestionCategory')->where(array('status' => 1, 'id' => $v['category']))->find();
            $v['description']=parse_expression($v['description']);
          //  $v['description']=M('ContentHandler')->limitPicture($v['description'],1);
        ...
        $parent =  M('QuestionCategory')->where(array('status' => 1,'pid'=>0))->order('sort asc')->select();
        foreach($parent as &$v){
            $v['child'] =  M('QuestionCategory')->where(array('status' => 1,'pid'=>$v['id']))->order('sort asc')->select();
        }
        unset($v);
    ...
    public function questionDetail($id = 0)
    {
        ...
        $questionDetail['category'] = M('QuestionCategory')->where(array('status' => 1, 'id' => $questionDetail['category']))->find();
        ...
        if ($questionDetail['best_answer']) {
            $questionBestAnswer = M('QuestionAnswer')->where(array('id' => $questionDetail['best_answer'], 'status' => 1))->find();

        }
        $map['id'] = array('neq', $questionDetail['best_answer']);
        $questionComment = M('QuestionAnswer')->where(array('status' => 1, 'question_id' => $questionDetail['id'], $map))->order('support desc,create_time desc')->page($aPage, $aCount)->select();


        $totalCount = M('QuestionAnswer')->where(array('status' => 1, 'question_id' => $questionDetail['id']))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
        ...
        foreach ($questionComment as &$v) {
            ...
            $v['has_support']=M('QuestionSupport')->where(array('uid' => is_login(), 'row' => $v['id']))->find();        //判断是否支持过或者贬低过
        }

        $questionBestAnswer['user'] = query_user(array('nickname', 'avatar32', 'uid', 'space_mob_url'), $questionBestAnswer['uid']);

        $questionBestAnswer['support_user']=M('QuestionSupport')->where(array('row' => $questionBestAnswer['id'],'type'=>1))->select();//判断最佳回答有没有点赞
        foreach($questionBestAnswer['support_user'] as &$b){
            $b['user'] = query_user(array('nickname', 'avatar32', 'uid','space_mob_url'), $b['uid']);      //最佳回答点过赞的人
        }
        $questionBestAnswer['has_support']=M('QuestionSupport')->where(array('uid' => is_login(), 'row' => $questionBestAnswer['id']))->find();
...
//dump($questionDetail);exit;
    ...
    public function reply($id=0,$answerId=0,$isEdit=0)
    {
        if($isEdit==0){
            ...
            $answerContent= M('QuestionAnswer')->where(array('id' => $answerId, 'status' => 1))->find();
    ...
    public function addAnswer()
    {
        ...
        if ($aAnswerId) {
            ...
            $now_answer = M('Mob/QuestionAnswer')->getData(array('id' => $aAnswerId, 'status' => 1));
        ...
        $res = M('Mob/QuestionAnswer')->editData($data);
        if ($res) {
            //发送消息
        //    $messageModel = M('Common/Message');
             ...
             */
            $question = M('Mob/Question')->find($aQuestion);
           // $messageModel->sendMessage($question['uid'], $user_info['nickname'] . '回答了你的问题【' . $question['title'] . '】或编辑了 Ta 的答案，快去看看吧！', '问题被回答', U('Question/Index/detail', array('id' => $aQuestion)), is_login(), 1);
            M('Common/Message')->sendMessage($question['uid'],'问题被回答', $user_info['nickname'] . '回答了你的问题【' . $question['title'] . '】或编辑了 Ta 的答案，快去看看吧！',  'Question/index/detail',array('id' =>$aQuestion), is_login(), 1);
            //发送消息 end
            $result['status'] = 1;
            if ($aAnswerId) {
    ...
    public function setBestAnswer()
    {
        ...
        $question = M('Mob/Question')->getData($aQuestion);
        ...
        if ($question && $aAnswerId) {
            ...
            $result = M('Mob/Question')->editData(array('id' => $aQuestion, 'best_answer' => $aAnswerId));
            if ($result) {
                $res['status'] = 1;
                $tip = '在问题【' . $question['title'] . '】中你的回答被设为最佳答案。';
                $answer = M('Mob/QuestionAnswer')->getData(array('id' => $aAnswerId));
                M('Common/Message')->sendMessage($answer['uid'], '答案被设为最佳答案', $tip, 'Question/index/detail', array('id' => $aQuestion), is_login(), 1);
            } else {
                $res['info'] = '操作失败！';
            }
        } else {
    ...
    public function addQuestion()
    {
        ...
        if(IS_POST){
            ...
            $aId=I('id',0,'intval');
            if($aId){
                $data=M('Mob/Question')->getData($aId);
                $this->checkAuth('Question/Index/edit',$data['uid'],'没有编辑该问题权限！');
                $need_audit=modC('QUESTION_NEED_AUDIT',1,'Question');
                if($need_audit){
                    $data['status']=2;
                }
                $questionDetail = M('Question')->where(array('status' => 1, 'id' => $aId))->order('create_time desc')->find();
                $questionDetail['category'] = M('QuestionCategory')->where(array('status' => 1, 'id' => $questionDetail['category']))->find();
            ...
            $this->setTopTitle('提问');
            $category=M('Mob/QuestionCategory')->getCategoryList(array('status'=>1),1);
    ...
    public function support()
    {
        ...
        $answer=M('Mob/QuestionAnswer')->getData(array('id'=>$aId,'status'=>1));
        if($answer['uid']==get_uid()){
            $res['info']="不能支持、反对自己的回答！";
            $this->ajaxReturn($res);
        }
        if(!(M('Mob/questionSupport')->where(array('uid'=>get_uid(),'tablename'=>'QuestionAnswer','row'=>$aId))->count())){
            $resultAdd=M('Mob/questionSupport')->addData('QuestionAnswer',$aId,$aType);
        }else{
            $res['info']="你已经支持或反对过该回答，不能重复操作！";
            $this->ajaxReturn($res);
        }
        if($resultAdd){
            $result=M('Mob/questionAnswer')->changeNum($aId,$aType);
        }
        if($result){
            //发送消息
            $question=M('Mob/question')->find($answer['question_id']);
            if($aType){
                $user_info=query_user(array('nickname','uid'));
             ...
             * @param int $type 消息类型，0系统，1用户，2应用
             */
            //M('Common/Message')->sendMessage($answer['uid'],$title, $tip , 'Question/index/Questiondetail',array('id'=>$answer['question_id']), 0, 1);
            M('Common/Message')->sendMessage($answer['uid'],$title, $tip,  'Question/index/detail',array('id' => $answer['question_id']), is_login(), 1);
            //发送消息 end
