...
class WeiboController extends BaseController
{
    ...
    public function index()
    {
        ...
        $totalCount = M('Weibo')->where(array('status' => 1))->count();
        $weibo = M('Weibo')->where(array('status' => 1,))->page($aPage, $aCount)->order('create_time desc')->select();


        $support['appname'] = 'Weibo';                              //查找是否点赞
        $support['table'] = 'weibo';
        $support['uid'] = is_login();
        $is_zan = M('Support')->where($support)->select();
        $is_zan = array_column($is_zan, 'row');

        foreach ($weibo as &$v) {
            $v['user'] = query_user(array('nickname', 'avatar64', 'uid'), $v['uid']);
            $v['rand_title'] = mob_get_head_title($v['uid']);
            $v['support'] = M('Support')->where(array('appname' => 'Weibo', 'table' => 'weibo', 'row' => $v['id']))->count();
         ...
         //   $v['content'] = parse_weibo_mobile_content($v['content']);
            ...
            $v['data'] = unserialize($v['data']);              //字符串转换成数组,获取轻博客源ID
            if ($v['data']['sourceId']) {                        //判断是否是源轻博客
            ...
            $v['sourceId_user'] = M('Weibo')->where(array('status' => 1, 'id' => $v['sourceId']))->find();           //源轻博客用户名
            $v['sourceId_user'] = $v['sourceId_user']['uid'];
            $v['sourceId_user'] = query_user(array('nickname', 'uid'), $v['sourceId_user']);

            $v['sourceId_content'] = M('Weibo')->where(array('status' => 1, 'id' => $v['sourceId']))->field('content')->find();          //源轻博客内容
           if(!is_null($v['sourceId_content'])){
               $v['sourceId_content'] = parse_weibo_mobile_content($v['sourceId_content']['content']);                                          //把表情显示出来。
           }


            $v['sourceId_repost_count'] = M('Weibo')->where(array('status' => 1, 'id' => $v['sourceId']))->field('repost_count')->find();    //源轻博客转发数
            $v['sourceId_from'] = M('Weibo')->where(array('status' => 1, 'id' => $v['sourceId']))->field('from')->find();       //源轻博客来源
            ...
            $v['sourceId_img'] = M('Weibo')->where(array('status' => 1, 'id' => $v['sourceId']))->field('data')->find();    //为了获取源微图片
...
//获得原图
                $bi = D('Picture')->where(array('status' => 1))->getById($b);
...
//获得原图
    ...
    public function weiboDetail($id)
    {
        $aPage = I('post.page', 1, 'op_t');
        $aCount = I('post.count', 10, 'op_t');
        $this->setTopTitle('轻博客详情');
        $weibodetail = M('Weibo')->where(array('id'=>$id,'status'=>1))->find();
        ...
        $weibodetail['support'] = M('Support')->where(array('appname' => 'Weibo', 'table' => 'weibo', 'row' => $weibodetail['id']))->count();


        $weibodetail['data'] = unserialize($weibodetail['data']);              //字符串转换成数组,获取轻博客源ID
        ...
        $weibodetail['sourceId_user'] = M('Weibo')->where(array('status' => 1, 'id' => $weibodetail['sourceId']))->find();           //源轻博客用户名
        ...
        $weibodetail['sourceId_content'] = M('Weibo')->where(array('status' => 1, 'id' => $weibodetail['sourceId']))->field('content')->find();          //源轻博客内容
        ...
        $weibodetail['sourceId_repost_count'] = M('Weibo')->where(array('status' => 1, 'id' => $weibodetail['sourceId']))->field('repost_count')->find();    //源轻博客转发数
        $weibodetail['sourceId_from'] = M('Weibo')->where(array('status' => 1, 'id' => $weibodetail['sourceId']))->field('from')->find();       //源轻博客来源
        ...
        $weibodetail['sourceId_img'] = M('Weibo')->where(array('status' => 1, 'id' => $weibodetail['sourceId']))->field('data')->find();    //为了获取源微图片
        ...
        foreach ($weibodetail['sourceId_img'] as &$b) {                                     //取得转发后源轻博客图片
        ...
        foreach ($weibodetail['cover_url'] as &$a) {                                   //取得转发的轻博客的图片
        ...
        if (empty($weibodetail['sourceId_img']['0'])) {                     //判断源轻博客是否有图片
        ...
        $weibocomment = M('Weibo_comment')->where(array('status' => 1, $mapl))->page($aPage, $aCount)->order('create_time desc')->select();
        $totalCount = M('Weibo_comment')->where(array('status' => 1, $mapl))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
...
//dump($weibodetail);exit;
        ...
        $this->assign('weibocomment', $weibocomment);                //轻博客评论
    ...
    public function addMoreComment()
    {
        ...
        $weibocomment = M('WeiboComment')->where(array('status' => 1, $map))->page($aPage, $aCount)->order('create_time desc')->select();
 ...
 */
    ...
    public function myFocus()
    {
        ...
        $follow_who_ids = M('Follow')->where(array('who_follow' => is_login()))->field('follow_who')->select();
        $follow_who_ids = array_column($follow_who_ids, 'follow_who');//简化数组操作。
        $follow_who_ids = array_merge($follow_who_ids, array(is_login()));//加上自己的轻博客
        $map['uid'] = array('in', $follow_who_ids);
        $weibo = M('Weibo')->where(array('status' => 1, $map))->page($aPage, $aCount)->order('create_time desc')->select();//我关注的人的轻博客
        $totalCount = M('Weibo')->where(array('status' => 1, $map))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
    ...
    public function addMoreMyFocus()
    {
        ...
        $follow_who_ids = array_merge($follow_who_ids, array(is_login()));//加上自己的轻博客
        $map['uid'] = array('in', $follow_who_ids);
        $weibo = M('Weibo')->where(array('status' => 1, $map))->page($aPage, $aCount)->order('create_time desc')->select();
...
//获得原图
    ...
    public function support($id, $uid)
    {
        ...
        if (M('Support')->where($support)->count()) {
            ...
            if (M('Support')->where($support)->add($support)) {
                ...
                M('Common/Message')->sendMessage($message_uid, $user['username'] . '给您点了个赞。', $title = $user['username'] . '赞了您。', 'Weibo/Index/weiboDetail',array('id' => $id), is_login(), 1);
                $return['status'] = '1';
            } else {
                $return['status'] = ' 0';
                $return['info'] = '亲，您已经支持过我了！';
    ...
    public function forward($id, $uid)
    {
        ...
        $map['id'] = array('eq', $id);
        $weibo = M('Weibo')->where(array('status' => 1, $map))->order('create_time desc')->select();
        // dump($weibo);
        ...
        foreach ($weibo as &$v) {
            ...
            $v['sourceId_user'] = $v['sourceId_user']['uid'];
            $v['sourceId_user'] = query_user(array('nickname', 'uid'), $v['sourceId_user']);
            $v['sourceId_content'] = M('Weibo')->where(array('status' => 1, 'id' => $v['sourceId']))->field('content')->find();
        }
    ...
    public function  doForward()
    {
        ...
        $aSoueseId = I('post.sourceId', 0, 'intval');           //获取该轻博客源ID
        $aWeiboId = I('post.weiboId', 0, 'intval');             //要转发的轻博客的ID
        ...
        $this->checkAuth('Weibo/Index/doSendRepost', -1, '您无轻博客转发权限。');
        ...
        $new_id = send_mob_weibo($aContent, $aType, $feed_data, $from = '手机网页版');        //发布轻博客


        if ($new_id) {
            M('weibo')->where('id=' . $aSoueseId)->setInc('repost_count');
            $aWeiboId != $aSoueseId && M('weibo')->where('id=' . $aWeiboId)->setInc('repost_count');
          //  S('weibo_' . $aWeiboId, null);
          //  S('weibo_' . $aSoueseId, null);
        }
// 发送消息
        $user = query_user(array('nickname', 'uid'), is_login());
        $toUid = M('weibo')->where(array('id' => $aWeiboId))->getField('uid');
        M('Common/Message')->sendMessage($toUid, '转发提醒' ,  $user['nickname'] . '转发了您的轻博客！',  'Weibo/Index/weiboDetail',array('id' => $new_id), is_login(), 1);
        // 发布评论
        ...
        //转发后的轻博客内容获取
        $weibo = M('Weibo')->where(array('status' => 1, 'id' => $new_id))->order('create_time desc')->select();
        $support['appname'] = 'Weibo';                              //查找是否点赞
        $support['table'] = 'weibo';
        $support['uid'] = is_login();
        $is_zan = M('Support')->where($support)->select();
...
//dump($weibo);exit;
    ...
    public function doAddComment()
    {
        ...
        $aWeiboId = I('post.weiboId', 0, 'intval');             //要评论的轻博客的ID
        ...
        $this->checkAuth('Weibo/Index/doComment', -1, '您无轻博客评论权限。');
        ...
        $weibocomment = M('WeiboComment')->where(array('status' => 1, 'id' => $new_id))->order('create_time desc')->select();
    ...
    public function delComment()
    {
        ...
        $weibo_id = I('post.weiboId', 0, 'intval');                   //接收轻博客ID

        $weibo_uid = M('Weibo')->where(array('status' => 1, 'id' => $weibo_id))->find();//根据轻博客ID查找轻博客发送人的UID
        $comment_uid = M('WeiboComment')->where(array('status' => 1, 'id' => $comment_id))->find();//根据评论ID查找评论发送人的UID
        ...
        if (is_administrator(get_uid()) || $weibo_uid['uid'] == get_uid() || $comment_uid['uid'] == get_uid()) {                                     //如果是管理员，则可以删除评论
            $result = M('WeiboComment')->deleteComment($comment_id);
        }
        if ($result) {
            $return['status'] = 1;
        } else {
    ...
    public function hotWeibo()
    {
        $aPage = I('post.page', 1, 'op_t');
        $aCount = I('post.count', 10, 'op_t');
        $this->setTopTitle('热门动态'); //$this->setTopTitle('热门轻博客');
        $hot_left = modC('HOT_LEFT', 3);
        $time_left = get_some_day($hot_left);
        $param['create_time'] = array('gt', $time_left);
        $param['status'] = 1;
        $param['is_top'] = 0;
        $weibo = M('Weibo')->where(array('status' => 1, $param))->page($aPage, $aCount)->order('comment_count desc')->select();
        $totalCount = M('Weibo')->where(array('status' => 1, $param))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
