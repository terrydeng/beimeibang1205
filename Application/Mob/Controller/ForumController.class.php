...
class ForumController extends BaseController
{
    ...
    public function index()
    {
        ...
        foreach ($list_top as &$v) {
            ...
            $v['plate'] = M('Forum')->where(array('status' => 1,'id'=>$v['forum_id']))->find();
        }
        unset($v);
        $this->assign('list_top', $list_top);


        $forum = M('ForumPost')->where(array('status' => 1))->page($aPage, $aCount)->order($order)->select();
        foreach($forum as &$v){
            $v['user']=query_user(array('nickname', 'avatar32'), $v['uid']);
        ...
        $totalCount =  M('ForumPost')->where(array('status' => 1))->count();
    ...
    public function forumtype(){
        $forum_top = M('ForumType')->where(array('status' => 1))->select();        //查找顶级分类pid=0的

        //  dump($issue_top);exit;
        foreach ($forum_top as &$v) {
            $v['lever_two'] = M('Forum')->where(array('status' => 1, 'type_id' => $v['id']))->select();        //查找二级分类pid=$issue_top的id
            $v['count'] = count($v['lever_two']);                //二级分类数量
            foreach ($v['lever_two'] as &$k) {
                $k['count_content'] = M('ForumPost')->where(array('status' => 1, 'forum_id' => $k['id']))->count();
            }
        }
    ...
    public function addComment($id=0,$is_edit=0,$reply_id=0){
        if($is_edit==1){
            ...
            if ($reply_id) {
                $reply = M('forum_post_reply')->where(array('id' => $reply_id, 'status' => 1))->find();
                $reply['is_edit']=1;
            } else {
                $this->error('参数出错！');
            }
    ...
    public function doSendLZLReply($p=1)
    {
        ...
        //显示成功页面
        $totalCount = M('forum_lzl_reply')->where('is_del=0 and to_f_reply_id=' . $to_f_reply_id)->count();
        $limit = 5;
    ...
    public function delLZLReply(){
        ...
        $Lzlreply=M('ForumLzlReply')->where('id='.$id)->find();
        $data['post_reply_id']=$Lzlreply['to_f_reply_id'];
        $res= M('ForumLzlReply')->delLZLReply($id);
        $data['lzl_reply_count']=M('ForumLzlReply')->where('is_del=0 and to_f_reply_id='.$data['post_reply_id'])->count();
    ...
    public function  get_expect_ids($lzl_reply_id=0,$reply_id=0,$post_id=0,$forum_id=0,$with_self=1)
    {
        $uid=0;
        if($lzl_reply_id){
            $lzl_reply=M('ForumLzlReply')->find($lzl_reply_id);
            $uid=$lzl_reply['uid'];
            $post_id=$lzl_reply['post_id'];
        }
        if(!$uid){
            if($reply_id){
                $reply = M('ForumPostReply')->find(intval($reply_id));
                $uid=$reply['uid'];
                $post_id=$reply['post_id'];
            }
        }
        if($post_id){
            $post=M('ForumPost')->where(array('id' => $post_id, 'status' => 1))->find();
            $forum_id=$post['forum_id'];
            if(!$uid){
                $uid=$post['uid'];
            }
        }
        $forum=M('Forum')->find($forum_id);
    ...
    public function postSectionDetail($id){
        ...
        $forum= M('ForumPost')->where(array('status' => 1, 'forum_id' => $id))->page($aPage, $aCount)->order($order)->select();

        foreach ($forum as &$v) {
            $v['user'] = query_user(array('nickname', 'avatar32'), $v['uid']);
            $v['plate'] = M('Forum')->where(array('status' => 1,'id'=>$v['forum_id']))->find();
        }
        $title= M('Forum')->where(array('status' => 1, 'id' => $id))->find();
        $title['pid']=1;                    //设置一个标记，执行过这个function的增加这个标记，页面进行判断。
        $totalCount =  M('ForumPost')->where(array('status' => 1, 'forum_id' => $id))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
    ...
    public function addMorePostSectionDetail(){
        ...
        $forum= M('ForumPost')->where(array('status' => 1, 'forum_id' => $aId))->page($aPage, $aCount)->order($order)->select();
    ...
    //    $replyList = M('ForumPostReply')->getReplyList($mapl, 'create_time', $aPage, $aCount);

        M('ForumPost')->where(array('id' => $id))->setInc('view_count');//查看数加1
        $is_add=M('ForumBookmark')->where(array('post_id' => $id,'uid'=>is_login()))->find();       //是否收藏
        if(is_null($is_add)){
            $is_add=0;
        }else{
            $is_add=1;
        ...
        $forum_detail['support']=M('Support')->where(array('appname'=>'Forum','row'=>$forum_detail['id']))->count();
        $forum_detail['has_support']= M('Support')->where(array('appname'=>'Forum','row'=>$forum_detail['id'],'uid'=>is_login()))->select();              //判断是否已经点赞
        ...
        $post_detail= M('ForumPostReply')->where(array('status'=>'1','post_id' => $id))->page($aPage, $aCount)->select();
        foreach($post_detail as &$v){
            ...
            //楼中楼内容
            $v['lzllist']= $list = M('Mob/ForumLzlReply')->getLZLReplyList($v['id'],'ctime asc',$aLzlPage,$aLzlCount);
//dump($v['lzllist']);exit;

            $v['lzltotalCount']= $totalCountLzl = M('forum_lzl_reply')->where('is_del=0 and to_f_reply_id=' . $v['id'])->count();
        ...
        $totalCount = M('ForumPostReply')->where(array('status'=>'1','post_id' => $id))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
    ...
    public function addMoreForumComment(){
        ...
        $forum_detail = M('ForumPost')->where($map)->find();
        $forum_detail['user']=query_user(array('nickname', 'avatar128'), $forum_detail['uid']);
        $post_detail= M('ForumPostReply')->where(array('status'=>'1','post_id' => $aId))->page($aPage, $aCount)->select();
        foreach($post_detail as &$v){
            $v['user']=query_user(array('nickname', 'avatar32'), $v['uid']);
            $v['content']=parse_expression($v['content']);
            if($forum_detail['uid']==$v['uid']){
         ...
         //   dump($v['id']);exit;
            $v['lzllist']= $list = M('Mob/ForumLzlReply')->getLZLReplyList($v['id'],'ctime asc',$aLzlPage,$aLzlCount);
            $v['lzltotalCount']= $totalCount = M('forum_lzl_reply')->where('is_del=0 and to_f_reply_id=' . $v['id'])->count();
            $data['to_f_reply_id'] = $v['id'];
...
public function support($id=0, $uid=0){
    ...
    {
        $id = intval($id);
        $post=M('ForumPost')->where(array('id' => $id, 'status' => 1))->find();
        $forum_id=$post['forum_id'];
        ...
        $res = D('ForumPost')->where(array('id'=>$id))->setField('status',-1);
        if($res){
            $return['status']=1;
        }else {
            $return['status'] = 0;
    ...
    if($isedit==0){
      $this->setTopTitle('发帖子');
       ...
       $this->setTopTitle('编辑帖子');
        ...
        $forum_detail = M('ForumPost')->where($map)->find();        $forum_detail['user']=query_user(array('nickname', 'avatar128'), $forum_detail['uid']);
        $forum_detail['content']=$content = parse_expression($forum_detail['content']);
    ...
    private function isForumExists($forum_id)
    {
        ...
        $forum = M('Forum')->where(array('id' => $forum_id, 'status' => 1));
    ...
    private function isForumAllowCurrentUserGroup($forum_id)
    {
        ...
        //读取用户所在的用户组
        $list = D('AuthGroupAccess')->where(array('uid' => is_login()))->select();
        foreach ($list as &$e) {
            $e = $e['group_id'];
        }
    ...
    public function delPostReply()
    {
        ...
        $res = M('Mob/ForumPostReply')->delPostReply($id);
        if($res==1){
            $return['status'] = 1;
        }else{
            $return['status'] = 0;
    ...
    {
        $post_id = intval($post_id);
        $post = M('ForumPost')->where(array('id' => $post_id))->find();
        if (!$post) {
            $this->error('帖子不存在');
        }
    }
    ...
    public function collection(){
        ...
        //写入数据库
        if ($add) {
            $result = M('Forum/ForumBookmark')->addBookmark(is_login(), $post_id);
            if (!$result) {
                $this->error('收藏失败');
            }
        } else {
            $result = M('Forum/ForumBookmark')->removeBookmark(is_login(), $post_id);
            if (!$result) {
                $this->error('取消失败');
            }
        }
