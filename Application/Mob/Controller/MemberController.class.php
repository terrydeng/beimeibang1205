...
/*            $register_type = modC('REGISTER_TYPE', 'normal', 'Invite');
            ...
            if (($aRegType == 'mobile' && modC('MOBILE_VERIFY_TYPE', 0, 'USERCONFIG') == 1) || (modC('EMAIL_VERIFY_TYPE', 0, 'USERCONFIG') == 2 && $aRegType == 'email')) {
                if (!M('Verify')->checkVerify($aUsername, $aRegType, $aRegVerify, 0)) {
            ...
            if (0 < $uid) { //注册成功
                ...
                if (modC('EMAIL_VERIFY_TYPE', 0, 'USERCONFIG') == 1 && $aUnType == 2) {
                    ...
                    $verify = M('Verify')->addVerify($email, 'email', $uid);
                ...
                $res = M('Mob/Member')->mobileLogin($uid, false, $aRole); //登陆
                if ($res) {
    ...
    public function step()
    {
        ...
        $userRoleModel->where($map)->setField('step', $aStep);
        if ($aStep == 'finish') {
            M('Member')->login($aUid, false, $aRoleId);
        }
        $this->assign('step', $aStep);
    ...
    public function inCode()
    {
        if (IS_POST) {
            ...
            $invite = M('Mob/Invite')->getByCode($aCode);
            if ($invite) {
    ...
    public function upRole()
    {
        ...
        if (IS_POST) {
            ...
            if ($uid > 0 && $aRoleId != get_login_role()) {
                ...
                $invite = M('Ucenter/Invite')->getByCode($aCode);
                if ($invite) {
                    if ($invite['end_time'] > time()) {
                        $map['id'] = $invite['invite_type'];
                        $map['roles'] = array('like', '%[' . $aRoleId . ']%');
                        $invite_type = M('Ucenter/InviteType')->getSimpleData($map);
                        if ($invite_type) {
    ...
    private function activateVerify()
    {
        ...
        $verify = M('Verify')->addVerify($email, 'email', $aUid);
    ...
    public function saveAvatar()
    {
        ...
        $res = D('avatar')->where(array('uid' => $aUid))->save(array('uid' => $aUid, 'status' => 1, 'is_temp' => 0, 'path' => "/" . $aUid . "/crop." . $aExt, 'create_time' => time()));
        if (!$res) {
            D('avatar')->add(array('uid' => $aUid, 'status' => 1, 'is_temp' => 0, 'path' => "/" . $aUid . "/crop." . $aExt, 'create_time' => time()));
        }
    ...
    public function doActivate()
    {
        ...
        $aType = I('get.type', '', 'op_t');
        $aUid = I('get.uid', 0, 'intval');
        $check = M('Verify')->checkVerify($aAccount, $aType, $aVerify, $aUid);
        if ($check) {
                ...
                if ($invite['end_time'] <= time()) {
                    ...
                    if ($invite_type) {
                        if (count($invite_type['roles'])) {
                            ...
                            $roleList = M('Admin/Role')->selectByMap($map_role, 'sort asc', 'id,title');
                            if (!count($roleList)) {
    ...
    private function checkInviteCode($code = '')
    {
        ...
        $invite = M('Ucenter/Invite')->getByCode($code);
        if ($invite['end_time'] >= time()) {
            $map['id'] = $invite['invite_type'];
            $invite_type = M('Ucenter/InviteType')->getSimpleData($map);
            if ($invite_type) {
    ...
    {
        if ($code != '') {
            $inviteModel = M('Ucenter/Invite');
            $invite = $inviteModel->getByCode($code);
            $data['inviter_id'] = abs($invite['uid']);
            $data['uid'] = $uid;
            $data['invite_id'] = $invite['id'];
            $result = M('Ucenter/InviteLog')->addData($data, $role);
            if ($result) {
                M('Ucenter/InviteUserInfo')->addSuccessNum($invite['invite_type'], abs($invite['uid']));
                ...
                if ($invite_type['is_follow']) {
                    $followModel = M('Common/Follow');
                    $followModel->addFollow($uid, abs($invite['uid']));
                    $followModel->addFollow(abs($invite['uid']), $uid);
                }
                if ($invite['uid'] > 0) {
                    M('Ucenter/Score')->setUserScore(array($invite['uid']), $invite_type['income_score'], $invite_type['income_score_type'], 'inc');//扣积分
                }
            }
        }
        return true;
