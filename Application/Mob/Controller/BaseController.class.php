...
/*
        if($this->is_weixin()){
            ...
            if ($moduleModel->checkInstalled('Weixin')) {
                $config = M('Weixin/WeixinConfig')->getWeixinConfig();
        ...
        $this->_top_menu_list =array(
            ...
            'center'=>array('title'=>'微1博02','h1_class'=>'','i_class'=>'') //'center'=>array('title'=>'轻博客','h1_class'=>'','i_class'=>'')
        );
        if(is_login()){
            if(check_auth('Weibo/Index/doSend')){
                $this->_top_menu_list['right'][]=array('type'=>'edit','href'=>U('Mob/Weibo/addWeibo'),'a_class'=>'','i_class'=>'');
            }else{
                $this->_top_menu_list['right'][]=array('type'=>'edit','info'=>'你没有权限发布轻博客！','a_class'=>'','i_class'=>'');
            }
        }else{
    ...
    public function getLocalCommentList($path, $page = 1)
    {
        ...
        $aMod = $aPath[1];
        $aRowId = $aPath[2];
        $model = M('Addons://LocalComment/LocalComment');
    ...
    public function delLocalComment()
    {
        ...
        if ($res) {
            $aCountModel && $aCountField && M($aCountModel)->where(array('id' => $comment['row_id']))->setDec($aCountField);
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
        }
