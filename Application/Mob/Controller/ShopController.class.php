...
class ShopController extends BaseController
{
    ...
    public function index($mark = 'allGoods', $typeId = 0)
    {
        ...
        switch ($mark) {
            case 'allGoods':
                ...
                $goods = M('Shop')->where(array('status' => 1))->order('createtime desc')->page($aPage, $aCount)->select();
                $totalCount = M('Shop')->where(array('status' => 1))->count();
                break;
            case 'newGoods':
                $this->setTopTitle('最新商品');
                $goods = M('Shop')->where(array('status' => 1,'is_num'=>1))->order('createtime desc')->page($aPage, $aCount)->select();
                $totalCount = M('Shop')->where(array('status' => 1,'is_num'=>1))->count();
                break;
            case "hotGoods":
                $this->setTopTitle('最热商品');
                $map['sell_num']=array('egt',$hot_num);
                $goods = M('Shop')->where(array('status' => 1,$map))->order('createtime desc')->page($aPage, $aCount)->select();
                $totalCount = M('Shop')->where(array('status' => 1,$map))->count();
                break;
            case 'parentType':
                $this->assign('typeId', $typeId);
                $typeName['parent']=M('ShopCategory')->where(array('id'=>$typeId))->find();
                $typeName['child']=M('ShopCategory')->where(array('pid'=>$typeName['parent']['id']))->select();
                if(is_null( $typeName['child'])){
                    $this->setTopTitle($typeName['parent']['title']);
                    $goods = M('Shop')->where(array('category_id'=>$typeId,'status' => 1))->order('createtime desc')->page($aPage, $aCount)->select();
                    $totalCount = M('Shop')->where(array('category_id'=>$typeId,'status' => 1))->count();
                }else{
                    ...
                    $goods = M('Shop')->where($map,array('status' => 1))->order('createtime desc')->page($aPage, $aCount)->select();
                    $totalCount = M('Shop')->where($map,array('status' => 1))->count();
                }
                break;
            case 'childType':
                $this->assign('typeId', $typeId);
                $typeName=M('ShopCategory')->where(array('id'=>$typeId))->find();
        ...
        //获取分类
        $parent = M('ShopCategory')->where(array('status' => 1, 'pid' => 0))->order('create_time asc')->select();
        foreach ($parent as &$v) {
            $v['child'] = M('ShopCategory')->where(array('status' => 1, 'pid' => $v['id']))->order('create_time asc')->select();
        }
        unset($v);
        //获取分类END
        //获取判断是否最热条件并判断
    ...
    public function buy($id){
        $this->assign('id',$id);
        $address=M('ShopAddress')->where(array('uid'=>is_login()))->order('create_time desc')->find();
       // dump($address);exit;
    ...
    public function goodsBuy($id = 0, $name = '', $address = '', $zipcode = '', $phone = '', $address_id = '')
    {
        ...
        $goods = M('shop')->where(array('id'=>$id))->find();
        if ($goods) {
            if($num<=0){
            ...
            if ($address_id) {
                $address_save = M('shop_address')->where(array('id' => $address_id))->save($shop_address);
                if ($address_save) {
                    M('shop_address')->where(array('id' => $address_id))->setField('change_time', time());
                }
                $data['address_id'] = $address_id;
            } else {
                $shop_address['uid'] = is_login();
                $shop_address['create_time'] = time();
                $data['address_id'] = M('shop_address')->add($shop_address);
            }
            //验证结束
            ...
            $res = M('shop_buy')->add($data);
            if ($res) {
                //商品数量减少,已售量增加
                M('shop')->where('id=' . $id)->setDec('goods_num', $num);
                M('shop')->where('id=' . $id)->setInc('sell_num', $num);
                //发送系统消息
                $message = $goods['goods_name'] . "购买成功，请等待发货。";
                M('Common/Message')->sendMessageWithoutCheckSelf(is_login(),'购买成功通知', $message,  'Shop/Index/myGoods', array('status' => '0'));
    ...
    public function myOrder(){
        ...
        $hot_num = modC('SHOP_HOT_SELL_NUM', 10, 'Shop');//获取最热条件
        //未完成的订单
        $goodsBuy=M('ShopBuy')->where(array('uid'=>is_login(),'status'=>0))->order('createtime desc')->page($aPage, $aCount)->select();
        $totalCount = M('ShopBuy')->where(array('uid'=>is_login(),'status'=>0))->count();
        foreach($goodsBuy as &$v){
            $v['goods']=M('Shop')->where(array('id'=>$v['goods_id']))->find();
                if ($v['goods']['sell_num'] >= $hot_num) {
                    $v['goods']['is_hot'] = 1;
            }
        }unset($v);
        //已完成的订单
        $goodsHasBuy=M('ShopBuy')->where(array('uid'=>is_login(),'status'=>1))->order('createtime desc')->page($aPage, $aCount)->select();
        $totalCountHasBuy = M('ShopBuy')->where(array('uid'=>is_login(),'status'=>1))->count();
    ...
    public function goodsDetail($id=""){
        $goodsDetail=M('Shop')->where(array('id'=>$id))->find();
      //  dump($goodsDetail);exit;
        $this->assign('detail',$goodsDetail);

        $recommendGoods=M('Shop')->where(array('status'=>1))->order('sell_num desc,createtime desc')->limit(10)->select();
