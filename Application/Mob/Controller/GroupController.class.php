    ...
    public function index($mark = 'myGroup', $typeId = 0)
    {
        ...
        switch ($mark) {
            case 'myGroup':
                ...
                $group_ids = M('GroupMember')->getGroupIds(array('where' => array('uid' => is_login(), 'status' => 1)));
                $myattend = M('Group')->getList(array('where' => array('id' => array('in', $group_ids), 'status' => 1), 'page' => $aPage, 'count' => $aCount, 'order' => 'uid = ' . is_login() . ' desc ,uid asc'));
                foreach ($myattend as &$v) {
                    $v = M('Group')->getGroup($v);
                }
                unset($v);
                $posts = M('GroupPost')->where(array('group_id' => array('in', $group_ids), 'status' => 1))->page($aPage, $aCount)->order('create_time desc')->select();
                $totalCount = M('GroupPost')->where(array('group_id' => array('in', $group_ids), 'status' => 1))->count();
                $supportModel = M('Common/Support');
                foreach ($posts as &$v) {
                    $v['group'] = M('Group')->getGroup($v['group_id']);
            ...
            case 'discover':
                ...
                $posts = M('GroupPost')->where(array('status' => 1, 'group_id' => array('in', $group_ids)))->page($aPage, $aCount)->order('create_time desc')->select();
                $totalCount = M('GroupPost')->where(array('status' => 1, 'group_id' => array('in', $group_ids)))->count();
                $supportModel = M('Common/Support');
                foreach ($posts as &$v) {
                ...
                break;
            case 'select':
                $posts = M('GroupPost')->where(array('is_top' => 1, 'status' => 1))->page($aPage, $aCount)->order('create_time desc')->select();
                $totalCount = M('GroupPost')->where(array('is_top' => 1, 'status' => 1))->count();
                $supportModel = M('Common/Support');
                foreach ($posts as &$v) {
                    $v['support_count'] = $supportModel->getSupportCount('Group', 'post', $v['id']);
                    $v['summary']=parse_expression($v['summary']);
                ...
                break;
            case 'allGroup':
                $group = M('Group')->where(array('status' => 1))->order('create_time desc')->page($aPage, $aCount)->order('create_time desc')->select();
                $totalCount = M('Group')->where(array('status' => 1))->count();
                foreach ($group as &$v) {
            ...
            case 'parentType':
                ...
                $typeName['parent'] = M('GroupType')->where(array('id' => $typeId))->find();

                $typeName['child'] = M('GroupType')->where(array('pid' => $typeName['parent']['id']))->select();
                if (is_null($typeName['child'])) {
                    $this->setTopTitle($typeName['parent']['title']);
                    $Group = M('Group')->where(array('type_id' => $typeId, 'status' => 1))->order('create_time desc')->page($aPage, $aCount)->select();
                    $totalCount = M('Group')->where(array('type_id' => $typeId, 'status' => 1))->count();
                } else {
                    ...
                    $Group = M('Group')->where($map, array('status' => 1))->order('create_time desc')->page($aPage, $aCount)->select();

                    $totalCount = M('Group')->where($map, array('status' => 1))->count();
                }
                foreach ($Group as &$v) {
                    $v['user'] = query_user(array('nickname', 'avatar64','space_mob_url'), $v['uid']);
                    $v['logo'] = getThumbImageById($v['logo'], 200, 200);
            ...
            case 'childType':
                $this->assign('typeId', $typeId);
                $typeName = M('GroupType')->where(array('id' => $typeId))->find();
        ...
        $parent = M('GroupType')->where(array('status' => 1, 'pid' => 0))->order('sort asc')->select();
        foreach ($parent as &$v) {
            $v['child'] = M('GroupType')->where(array('status' => 1, 'pid' => $v['id']))->order('sort asc')->select();
        }
...
public function addMoreIndex(){
    ...
    switch ($aMark) {
        ...
        case 'discover':
            ...
            break;
        case 'select':
            $posts = M('GroupPost')->where(array('is_top' => 1, 'status' => 1))->order('create_time desc')->select();
            $supportModel = M('Common/Support');
            foreach ($posts as &$v) {
        ...
        case 'parentType':
            ...
            $typeName['parent'] = M('GroupType')->where(array('id' => $aTypeId))->find();

            $typeName['child'] = M('GroupType')->where(array('pid' => $typeName['parent']['id']))->select();
            if (is_null($typeName['child'])) {
                $this->setTopTitle($typeName['parent']['title']);
                $Group = M('Group')->where(array('type_id' => $aTypeId, 'status' => 1))->order('create_time desc')->page($aPage, $aCount)->select();
            } else {
                $typeId = array_column($typeName['child'], 'id');
        ...
        case 'childType':
            $this->assign('typeId', $aTypeId);
            $typeName = M('GroupType')->where(array('id' => $aTypeId))->find();
            $this->setTopTitle($typeName['title']);
            $Group = M('Group')->where(array('type_id' => $aTypeId, 'status' => 1))->order('create_time desc')->page($aPage, $aCount)->select();
            foreach ($Group as &$v) {
                $v['user'] = query_user(array('nickname', 'avatar64','space_mob_url'), $v['uid']);
    ...
    if(($aMark=='myGroup')||($aMark=='discover')||($aMark=='select')){
        foreach ($posts as &$v) {
            ...
            $v['count']=M('LocalComment')->where(array('app'=>'News','mod'=>'index','status'=>1,'row_id'=>$v['id']))->order('create_time desc')->count();
        }
        if ($posts) {
            $data['html'] = "";
    ...
    public function detail($id)
    {
        ...
        $aLzlCount = I('post.lzlcount', 3, 'op_t');
           //增加浏览次数
        M('GroupPost')->where(array('id' => $id))->setInc('view_count');
        //获取群组信息

        $group = M('GroupPost')->where(array('id' => $id))->find();
        $group = M('Group')->where(array('id' => $group['group_id']))->find();
        $group['logo'] = getThumbImageById($group['logo'], 100, 100);
        $group['member_count'] = M('GroupMember')->where(array('group_id' => $group['id']))->count();
        if ($group['type'] == 0) {
            $group['type'] = '公共群组';
        }
        if ($group['type'] == 1) {
        ...
        $group['GroupType'] = M('GroupType')->where(array('id' => $group['type_id']))->find();
        $group['notice'] = M('GroupNotice')->where(array('group_id' => $id))->find();

        //获取帖子信息
        $post = M('GroupPost')->where(array('id' => $id))->find();
        $post['collect']=M('GroupBookmark')->where(array('post_id'=>$post['id'],'uid'=>is_login()))->find();
        if($post['collect']){
            $post['collect']=1;
        }else{
            $post['collect']=0;
        }
        $post['countSupport']=M('Support')->where(array('appname'=>'Group','row'=>$id))->count();
        $post['isSupport']=M('Support')->where(array('appname'=>'Group','row'=>$id,'uid'=>is_login()))->find();
        if( $post['isSupport']){
            $post['isSupport']=1;
        }else{
            $post['isSupport']=0;
        ...
        //获取帖子评论信息
        $postComment = M('GroupPostReply')->where(array('post_id' => $id,'status'=>1))->page($aPage, $aCount)->select();
        $totalCount= M('GroupPostReply')->where(array('post_id' => $id,'status'=>1))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
...
//dump($postComment[0]['lzllist']);exit;
   // dump($id);exit;
    ...
    public function addMoreLzlreply(){
        ...
        $forum_detail = M('GroupPost')->where($map)->find();

        $forum_detail['user']=query_user(array('nickname', 'avatar128','space_mob_url'), $forum_detail['uid']);
        //楼中楼内容
        //   dump($v['id']);exit;
        $post_detail['lzllist']= $list = M('Mob/GroupLzlReply')->getLZLReplyList($aId,'create_time asc',$aLzlPage,$aLzlCount);
    ...
    public function group($id)
    {
        ...
        $group ['canAdmin']=M('GroupMember')->where(array('group_id' => $id,$map))->field('uid')->select();
        $group ['canAdmin']=array_column($group ['canAdmin'],'uid');


        //群组动态
        $list = M('GroupDynamic')->where(array('group_id' => $group['id']))->order('create_time desc')->select();
        $list = array_column($list, 'id');

        //获取帖子信息

        $post = M('GroupPost')->where(array('group_id' => $id))->order('create_time desc')->select();
        foreach ($post as & $v) {
            $v['user'] = query_user(array('nickname', 'avatar64', 'space_mob_url'), $v['uid']);
        }
        //成员
        $people['admin'] = M('GroupMember')->where(array('group_id' => $id, 'position' => 3))->order('create_time desc')->find();
        $people['admin']['user'] = query_user(array('nickname', 'avatar64', 'space_mob_url'), $people['admin']['uid']);
        $people['people'] = M('GroupMember')->where(array('group_id' => $id, 'position' => 1))->order('create_time desc')->select();
        // dump($people);exit;
    ...
    {
        $groupType = M('Group/GroupType')->getGroupTypes();
    ...
    public function attend()
    {
        ...
        $uid = is_login();
        $group = M('Mob/Group')->getGroup($aGroupId);
        ...
        if ($group['type'] == 1) {
            // 群组为私有的。
            $data['status'] = 0;
            $res = M('GroupMember')->addMember($data);
            $info = '，等待群组管理员审核！';
            // 发送消息
            M('Message')->sendMessage($group['uid'], '加入群组审核', get_nickname($uid) . "请求加入群组【{$group['title']}】", 'group/Manage/member', array('group_id' => $aGroupId, 'status' => 0), $uid);
        } else {
            // 群组为公共的
            $data['status'] = 1;
            $res = M('GroupMember')->addMember($data);
        ...
        if ($res) {
            M('Group')->where(array('id' => $aGroupId))->setInc('member_count');
    ...
    public function quit()
    {
        ...
        $res = M('GroupMember')->delMember(array('group_id' => $aGroupId, 'uid' => $uid));
        if ($res) {
            //添加到最新动态
            $dynamic['group_id'] = $aGroupId;
            $dynamic['uid'] = $uid;
            ...
            M('Group')->where(array('id' => $aGroupId))->setDec('member_count');
    ...
    public function invitationFriend()
    {
        ...
        if (IS_POST) {
            ...
            $group = M('Group')->getGroup($aGroupId);
            foreach ($uids as $uid) {
                M('Message')->sendMessage($uid, '邀请加入群组', get_nickname(is_login()) . "邀请您加入群组【{$group['title']}】  <a class='ajax-post' href='" . U('group/index/attend', array('group_id' => $aGroupId)) . "'>接受邀请</a>", 'group/index/group', array('id' => $aGroupId), is_login());
            }
            $result = array('status' => 1, 'info' => '邀请成功');
            $this->ajaxReturn($result);
        } else {
            $friendList = M('Follow')->getAllFriends(is_login());
    ...
    public function editCate()
    {
        ...
        if ($aCateId == 0) {
            $res = M('GroupPostCategory')->add(array('group_id' => $aGroupId, 'title' => $aTitle, 'create_time' => time(), 'status' => 1, 'sort' => 0));
            if (!$res) {
                $this->error('添加分类失败');
            }
            $this->success('添加分类成功', 'refresh');
        } else {
            $res = M('GroupPostCategory')->where(array('id' => $aCateId))->save(array('title' => $aTitle));
            if (!$res) {
                $this->error('编辑分类失败');
            }
    ...
    public function delCate()
    {
        ...
        $res = M('GroupPostCategory')->where(array('id' => $aCateId))->setField('status', 0);
        if (!$res) {
            $this->error('删除分类失败');
        }
    ...
    {
        $notice = M('GroupNotice')->getNotice($group_id);
    ...
    public function notice()
    {
        ...
        $res = M('GroupNotice')->addNotice($data);
        if ($res) {
    ...
    public function removeGroupMember()
    {
        ...
        $aGroupId = I('post.group_id', '', 'intval');
        $res = M('GroupMember')->where(array('uid' => $aUid, 'group_id' => $aGroupId))->delete();
        $dynamic['group_id'] =$aGroupId;
        $dynamic['uid'] = $aUid;
        $dynamic['type'] = 'remove';
        ...
        if ($res) {
            $group = M('Group')->getGroup($this->groupId);
            M('Message')->sendMessage($aUid, '移出群组', get_nickname(is_login()) . "将您移出了群组【{$group['title']}】", 'group/index/group', array('id' => $this->groupId), is_login());
            S('group_member_count_' . $group['id'],null);
    ...
    public function addComment($id=0,$is_edit=0,$reply_id=0){
        if($is_edit==1){
            ...
            if ($reply_id) {
                $reply = M('GroupPostReply')->where(array('id' => $reply_id, 'status' => 1))->find();
                $reply['is_edit']=1;
            } else {
                $this->error('参数出错！');
            }
    ...
    public function doReply()
    {
        ...
        if($isEdit==1){
            ...
            $groipReplyModel = M('GroupPostReply');
            ...
            D('GroupPost')->where(array('id' => $data['post_id']))->setInc('reply_count');
    ...
    {
        $post = M('GroupPost')->getPost($post_id);
...
//以下为楼中楼内容
    ...
    public function doSendLzlReply()
    {
        ...
        $model = M('Mob/GroupLzlReply');

        $reply = M('GroupPostReply')->getReply($aToFReplyId);
    ...
    public function delLzlReply()
    {
        ...
        $res = M('GroupLzlReply')->delLzlReply($aId);
    ...
    public function addPost($group_id = 0,$isEdit=0,$post_id=0)
    {
        ...
        if($isEdit!=0){
        $post=M('GroupPost')->where(array('id'=>$post_id))->find();
       ...
       // dump($group_id);exit;
        ...
        $cate = M('GroupPostCategory')->getList(array('where' => $map, 'order' => 'sort asc'));
        foreach ($cate as &$v) {
            $v = M('GroupPostCategory')->getPostCategory($v);
        }
    ...
    public function doBookmark()
    {
        ...
        //写入数据库
        if ($aFlag) {
            $result = M('GroupBookmark')->addBookmark(is_login(), $aPostId);
            if (!$result) {
                $this->error('收藏失败');
            }
        } else {
            $result = M('GroupBookmark')->removeBookmark(is_login(), $aPostId);
            if (!$result) {
                $this->error('取消失败');
            }
        }
    ...
    public function support($id=0, $uid=0){
        ...
        if (M('Support')->where($support)->count()) {
            $return['status'] = '0';
            $return['info'] = '亲，您已经赞过我了！';
        } else {
            $support['create_time'] = time();
            if (M('Support')->where($support)->add($support)) {

                $this->clearCache($support);

                $user = query_user(array('username', 'uid','space_mob_url'));

                M('Message')->sendMessage($message_uid, $user['username'] . '给您点了个赞。', $title = $user['username'] . '赞了您。', is_login());
