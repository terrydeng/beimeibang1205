...
class IssueController extends BaseController
{
    ...
    public function index()
    {
        ...
        $totalCount = M('Issue_content')->where(array('status' => 1))->count();
        $issue = M('Issue_content')->where(array('status' => 1,))->page($aPage, $aCount)->order('create_time desc')->select();
    ...
    public function issueDetail($id)
    {
        ...
        $issuedetail = M('Issue_content')->where($map)->find();

        $issuedetail['meta']['description'] = mb_substr($issuedetail['content'], 0, 50, 'UTF-8');//取得前50个字符
        $issuedetail['meta']['keywords'] = M('Issue')->where(array('id' => $issuedetail['issue_id']))->field('title')->find();

        $totalCount = M('Local_comment')->where(array('status' => 1, $mapl))->count();
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
        }
        $issuecomment = M('Local_comment')->where(array('status' => 1, $mapl))->page($aPage, $aCount)->order('create_time desc')->select();
        M('IssueContent')->where(array('id' => $id))->setInc('view_count');//查看数加1
        ...
        $issue_uid = M('IssueContent')->where(array('status' => 1, 'id' => $id))->find();//根据轻博客ID查找专辑发送人的UID
        ...
        //获得原图
        $bi = D('Picture')->where(array('status' => 1))->getById($issuedetail['cover_id']);
...
//dump($issuedetail);exit;
    ...
    public function addMoreIssueComment()
    {
        ...
        $issuecomment = M('Local_comment')->where(array('status' => 1, $map))->page($aPage, $aCount)->order('create_time desc')->select();
        foreach ($issuecomment as &$v) {
            $v['user'] = query_user(array('nickname', 'avatar32', 'uid'), $v['uid']);
            $v['cover_url'] = getThumbImageById($v['cover_id']);
            $v['content'] = parse_weibo_mobile_content($v['content']);
    ...
    public function support($id, $uid)
    {
        ...
        if (M('Support')->where($support)->count()) {
            ...
            if (M('Support')->where($support)->add($support)) {
                ...
                M('Common/Message')->sendMessage($message_uid, $user['username'] . '给您点了个赞。', $title = $user['username'] . '赞了您。', 'Issue/issue/weiboDetail',array('id' => $id), is_login(), 1);
                $return['status'] = '1';
            } else {
                $return['status'] = ' 0';
                $return['info'] = '操作失败！';
    ...
    public function doAddComment()
    {
        ...
        $uid = is_login();
        $result = M('LocalComment')->addIssueComment($uid, $aIssueId, $aContent);

        $title =get_nickname(is_login()). '评论了您';
        M('Common/Message')->sendMessage($aUid,$title, "评论内容：$aContent",  'Issue/Index/IssueContentDetail',array('id' => $aIssueId), is_login(), 0);
        action_log('add_issue_comment', 'local_comment', $result, $uid);

        $map['id'] = array('eq', $result);
        $issuecomment = M('Local_comment')->where(array('status' => 1, $map))->order('create_time desc')->select();
    ...
    public function delIssueComment()
    {
        ...
        $issue_uid = M('IssueContent')->where(array('status' => 1, 'id' => $issueId))->find();//根据轻博客ID查找专辑发送人的UID
        $comment_uid = M('LocalComment')->where(array('status' => 1, 'id' => $comment_id))->find();//根据评论ID查找评论发送人的UID
        if (!is_login()) {
        ...
        if (is_administrator(get_uid()) || $issue_uid['uid'] == get_uid() || $comment_uid['uid'] == get_uid()) {                                     //如果是管理员，则可以删除评论
            $result = M('LocalComment')->deleteComment($comment_id);
        }
        if ($result) {
            $return['status'] = 1;
        } else {
    ...
    public function atComment($id, $user, $uid)
    {
        ...
        //$uid是发帖人的ID
        $map['id'] = array('eq', $id);
        $issue = M('IssueContent')->where(array('status' => 1, $map))->select();
    ...
    {
        $issue_top = M('Issue')->where(array('status' => 1, 'pid' => 0))->select();        //查找顶级分类pid=0的

        //  dump($issue_top);exit;
        foreach ($issue_top as &$v) {
            $v['lever_two'] = M('Issue')->where(array('status' => 1, 'pid' => $v['id']))->select();        //查找二级分类pid=$issue_top的id
            $v['count'] = count($v['lever_two']);                //二级分类数量
            foreach ($v['lever_two'] as &$k) {
                $k['count_content'] = M('IssueContent')->where(array('status' => 1, 'issue_id' => $k['id']))->count();
            }
        }
    ...
    public function issueLeverDetail($id)
    {
        $aPage = I('post.page', 1, 'op_t');
        $aCount = I('post.count', 10, 'op_t');
        $totalCount = M('IssueContent')->where(array('status' => 1))->count();
        $issue = M('IssueContent')->where(array('status' => 1, 'issue_id' => $id))->page($aPage, $aCount)->select();        //查找顶级分类pid=0的
        if ($totalCount <= $aPage * $aCount) {
            $pid['count'] = 0;
        } else {
            $pid['count'] = 1;
        ...
        $title = M('Issue')->where(array('status' => 1, 'id' => $id))->find();        //查找顶级分类pid=0的
    ...
    public function addMoreIssueClass()
    {
        ...
        $aCount = I('post.count', 10, 'op_t');
        $aId = I('post.id', '', 'op_t');
        $issue = M('IssueContent')->where(array('status' => 1, 'issue_id' => $aId))->page($aPage, $aCount)->select();        //查找顶级分类pid=0的
    ...
    public function addIssue($id = 0)
    {
        ...
        $tree = M('Issue')->getTree();

        $issue = M('IssueContent')->where(array('status' => 1, 'id' => $id))->find();
    ...
    {
        $issues = M('Issue')->where(array('pid' => $pid, 'status' => 1))->limit(999)->select();
    ...
    public function doSendIssue()
    {
        ...
        $content = M('Issue/IssueContent')->create($data);
        ...
        if ($IssueId) {
            $content_temp = M('IssueContent')->find($IssueId);
            ...
            $rs = M('IssueContent')->save($content);
            //   dump(M('IssueContent')->getLastSql());exit;
            ...
            if (modC('NEED_VERIFY', 0) && !is_administrator()) //需要审核且不是管理员
            {
                ...
                foreach ($admin_uids as $admin_uid) {
                    M('Common/Message')->sendMessage($admin_uid, $title = '专辑投稿提醒',"{$user['nickname']}向专辑投了一份稿件，请到后台审核。",  'Admin/Issue/verify',array('id' => $IssueId), is_login(), 2);
                }
            }
            // dump($content);exit;
            $rs = M('IssueContent')->add($content);
            if ($rs) {
                $return['status'] = 1;
            } else {
                $return['status'] = 0;
