...
(function( $ ) {
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('uploadSuccess',function( file, response){
                alert(response.data.file.id);
				var $fileBox = $('#fileBox_'+file.id);
                alert($fileBox);
				var $diyBar = $fileBox.find('.diyBar');

                var src = response.data.file.path + '?time=' + response.data.file.time ;
                ...
                upAttachVal('add',response.data.file.id, $('#img_ids'));
	...
	function getOption(objId) {
		...
		return {
			...
			//服务器地址;
            server: "{:U('Core/File/stream2Image')}",
			...
			fileSingleSizeLimit: 2*500 * 1024 //fileSingleSizeLimit:500 * 1024
