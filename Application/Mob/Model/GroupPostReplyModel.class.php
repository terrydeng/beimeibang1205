...
class GroupPostReplyModel extends Model
{
    ...
    public function getReply($id)
    {
        ...
        if ($reply) {
            $reply['lzl_count'] = M('GroupLzlReply')->where(array('status' => 1, 'to_f_reply_id' => $reply['id']))->cache('group_lzl_count_' . $reply['id'], 60 * 5)->count();
        }
        return $reply;
    }
    ...
    public function delPostReply($id)
    {
        ...
        if ($res) {
            $lzlModel = M('Group/GroupLzlReply');
            $lzl = $lzlModel->getList(array('where' => array('to_f_reply_id' => $id, 'status' => 1)));
            foreach ($lzl as $v) {
                $lzlModel->delLzlReply($v);
            }
            M('GroupPost')->where(array('id' => $reply['post_id']))->setDec('reply_count');
