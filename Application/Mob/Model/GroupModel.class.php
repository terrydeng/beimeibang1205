...
class GroupModel extends Model
{
    ...
    public function getGroup($id)
    {
        ...
        if ($group) {
            $group['member_count'] = M('GroupMember')->where(array('group_id' => $group['id'], 'status' => 1))->cache('group_member_count_'.$group['id'],60*5)->count();
        }
    ...
    public function delGroup($id)
    {
        ...
        M('GroupPost')->where(array('group_id' => $id))->setField('status', -1);        //把该群组中的帖子删除。
        S('group_' . $id, null);
        return $res;
    }
