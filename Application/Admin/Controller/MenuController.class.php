...
class MenuController extends AdminController {
    ...
    public function add(){
        if(IS_POST){
            ...
            $menus = M('Common/Tree')->toFormatTree($menus);
            $menus = array_merge(array(0=>array('id'=>0,'title_show'=>L('_MENU_TOP_'))), $menus);
            $this->assign('Modules',M('Module')->getAll());
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = D('Menu')->where($map)->field('id,title')->order('sort asc,id asc')->select();
            ...
            foreach ($ids as $key=>$value){
                $res = D('Menu')->where(array('id'=>$value))->setField('sort', $key+1);
            }
            if($res !== false){
