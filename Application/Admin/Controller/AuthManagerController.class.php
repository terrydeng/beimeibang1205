...
class AuthManagerController extends AdminController
{
    ...
    public function user($group_id)
    {
        ...
        $model = D()->table($l_table . ' m')->join($r_table . ' a ON m.uid=a.uid');
        $_REQUEST = array();
    ...
    {
        $uid = I('uid');
        $auth_groups = M('AuthGroup')->getGroups();
        $user_groups = AuthGroupModel::getUserGroup($uid);
        $ids = array();
        foreach ($user_groups as $value) {
            $ids[] = $value['group_id'];
        }
        $nickname = M('Member')->getNickName($uid);
    ...
    public function addToGroup()
    {
        ...
        if (is_numeric($uid)) {
            ...
            if (!D('Member')->where(array('uid' => $uid))->find()) {
    ...
    public function deleteNode(){
        $aId=I('id',0,'intval');
        if($aId>0){
            $result=   D('AuthRule')->where(array('id'=>$aId))->delete();
            if($result){
    ...
    public function accessUser()
    {
        ...
        $this->updateRules();
        $auth_group = D('AuthGroup')->where(array('status' => array('egt', '0'), 'type' => AuthGroupModel::TYPE_ADMIN))
            ->getfield('id,id,title,rules');
        $node_list = $this->getNodeListFromModule(M('Common/Module')->getAll());

        //  $node_list   =D('AuthRule')->where(array('module'=>array('neq','admin'),'type'=>AuthRuleModel::RULE_URL,'status'=>1))->select();
    ...
    public function getNodeListFromModule($modules)
    {
        ...
        foreach ($modules as $module) {
            if ($module['is_setup']) {
                ...
                $node['child'] = D('AuthRule')->where($map)->select();
                $node_list[] = $node;
            }
