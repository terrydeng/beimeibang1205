...
class MessageController extends AdminController
{
    ...
    public function userList($page=1,$r=20)
    {
        ...
        if (empty($aSearch1) && empty($aSearch2)) {
            ...
            $user = M('member')->where($map)->page($page, $r)->field('uid,nickname')->select();
            foreach ($user as &$v) {
                $v['id'] = $v['uid'];
            }
            unset($v);
    ...
    private function getUids($user_group = 0, $role = 0)
    {
        $uids = array();
        if (!empty($user_group)) {
            $users = M('auth_group_access')->where(array('group_id' => $user_group))->field('uid')->select();
        ...
        if (!empty($role)) {
            $users = M('user_role')->where(array('role_id' => $role))->field('uid')->select();
    ...
    private function getUids_sc($search_nn = "", $search_id = 0)
    {
        $uids = array();
        if (!empty($search_nn)) {
            $users = M('member')->where(array('nickname' => $search_nn))->field('uid')->select();
        ...
        if (!empty($search_id)) {
            $users = M('member')->where(array('uid' => $search_id))->field('uid')->select();
    ...
    public function sendMessage()
    {
        ...
        if (IS_POST) {
            ...
            if(empty($aUids)){
                ...
                $role_count = M('Role')->where(array('status' => 1))->count();
                ...
                if (!empty($aUserRole)) {
                    $uids = M('user_role')->where(array('role_id' => array('in', $aUserRole)))->field('uid')->select();
                }
                if (!empty($aUserGroup)) {
                    $uids = M('auth_group_access')->where(array('group_id' => array('in', $aUserGroup)))->field('uid')->select();
                }
                if (empty($aUserRole) && empty($aUserGroup)) {
                    $uids = M('Member')->where(array('status' => 1))->field('uid')->select();
                }
            ...
            M('Message')->sendMessageWithoutCheckSelf($to_uids, $aTitle, $aContent, $aUrl, $args);
            $result['status'] = 1;
            ...
            if (empty($aUids)) {
                ...
                $uids = implode(',',$aUids);
                $users = M('Member')->where(array('uid'=>array('in',$aUids)))->field('uid,nickname')->select();
