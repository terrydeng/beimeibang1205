...
class ChannelController extends AdminController
{
    ...
    public function index()
    {
        ...
        if (IS_POST) {
            $one = $_POST['nav'][1];
            if (count($one) > 0) {
                D()->execute('TRUNCATE TABLE ' . C('DB_PREFIX') . 'channel');
    ...
    public function index1()
    {
        ...
        $list = D('Channel')->where($map)->order('sort asc,id asc')->select();
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            //获取父导航
            if (!empty($pid)) {
                $parent = D('Channel')->where(array('id' => $pid))->field('title')->find();
    ...
    public function sort()
    {
        if (IS_GET) {
            ...
            $list = D('Channel')->where($map)->field('id,title')->order('sort asc,id asc')->select();
            ...
            foreach ($ids as $key => $value) {
                $res = D('Channel')->where(array('id' => $value))->setField('sort', $key + 1);
            }
            if ($res !== false) {
