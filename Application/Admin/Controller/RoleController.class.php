...
class RoleController extends AdminController
{
    ...
    {
        parent::_initialize();
        $this->roleModel = M("Admin/Role");
        $this->userRoleModel = M('UserRole');
        $this->roleConfigModel = M('RoleConfig');
        $this->roleGroupModel = M('RoleGroup');
    }
    ...
    public function index($page = 1, $r = 20)
    {
        ...
        $authGroupList = D('AuthGroup')->where(array('status' => 1))->field('id,title')->select();
    ...
    public function editRole()
    {
        ...
        if (IS_POST) {
            ...
            $authGroupList = D('AuthGroup')->where(array('status' => 1))->field('id,title')->select(); //用户组列表

            $group = M('RoleGroup')->field('id,title')->select();
    ...
    private function checkSingleRoleUser($ids)
    {
        ...
        $user_ids=M('Member')->where(array('status'=>-1))->field('uid')->select();
    ...
    public function setUserStatus($ids, $status = 1, $role_id = 0)
    {
        ...
        if ($status == 1) {
            ...
            foreach($to_init_ids as $val){
                M('Common/Member')->initUserRoleInfo($role_id,$to_init_uids[$val]['uid']);
            }
            $builder = new AdminListBuilder;
    ...
    public function setUserAudit($ids,$status=1,$role_id=0)
    {
        ...
        if ($status == 1) {
            ...
            foreach ($ids as $val) {
                ...
                if($user_role['init']==0){
                    M('Common/Member')->initUserRoleInfo($role_id,$user_role['uid']);
                }
            }
            $builder = new AdminListBuilder;
            $builder->doSetStatus('UserRole', $ids, $status);
    ...
    public function configUserTag()
    {
        ...
        if(IS_POST){
            ...
            $tag_list=M('Ucenter/UserTag')->getTreeList();
    ...
    private function getExpendField($in = '')
    {
        ...
        $map['status'] = array('egt', 0);
        $profileList = M('field_group')->where($map)->order("sort asc")->select(); //获取扩展字段分组

        $fieldSettingModel = M('field_setting');
        $type_default = array(
