...
class RankController extends AdminController
{
    ...
    public function index($page = 1, $r = 20)
    {
        ...
        foreach ($list as &$val) {
            $val['u_name'] = M('member')->where('uid=' . $val['uid'])->getField('nickname');
    ...
    public function changeTypes($id = null)
    {
        ...
        $types = M('rank')->where('id=' . $id)->getField('types');
        $types = $types ? 0 : 1;
        $result = M('rank')->where('id=' . $id)->setField('types', $types);
        if ($result) {
    ...
    public function deleteRank($id = null)
    {
        ...
        $result = M('rank')->where('id=' . $id)->delete();
        $result1 = M('rank_user')->where('rank_id=' . $id)->delete();
        if ($result) {
    ...
    public function editRank($id = null)
    {
        ...
        if (IS_POST) {
            ...
            //如果是编辑模式
            if ($isEdit) {
                $rank = D('rank')->where(array('id' => $id))->find();
            }
            //显示页面
    ...
    public function userRankList($id = null, $page = 1)
    {
        ...
        $u_name = M('member')->where('uid=' . $id)->getField('nickname');
        $model = M('rank_user');
        ...
        foreach ($rankList as &$val) {
            $val['title'] = M('rank')->where('id=' . $val['rank_id'])->getField('title');
        ...
        if (IS_POST) {
            ...
            if ($is_Edit) {
                ...
                if (!$result) {
                    ...
                    $rank = M('rank')->where('id=' . $data['rank_id'])->find();
                    //$logoUrl=getRootUrl().M('picture')->where('id='.$rank['logo'])->getField('path');
                    //$u_name = M('member')->where('uid=' . $uid)->getField('nickname');
    ...
    public function userChangeRank($id = null, $uid = '', $reason = '', $is_show = '', $rank_id = '')
    {
        if (IS_POST) {
            ...
            $data = M('rank_user')->where('id=' . $id)->find();
            if (!$data) {
    ...
    public function deleteUserRank($id = null)
    {
        ...
        $result = M('rank_user')->where('id=' . $id)->delete();
        if ($result) {
            $this->success(L('_DELETE_SUCCESS_'));
        } else {
            $this->error(L('_DELETE_FAILED_'));
    ...
    {
        M('Message')->sendMessage($data['uid'], $type, $content, 'Ucenter/Message/message',array(),is_login(), 1);
    }
    ...
    public function rankVerify($page = 1)
    {
        ...
        foreach ($rankList as &$val) {
            ...
            $val['is_show'] = $val['is_show'] ? L('_SHOW_') : L('_NOT_SHOW_');
            //获取用户信息
            $u_user = M('member')->where('uid=' . $val['uid'])->getField('nickname');
            $val['u_name'] = $u_user;
        }
        unset($val);
        $builder = new AdminListBuilder();
