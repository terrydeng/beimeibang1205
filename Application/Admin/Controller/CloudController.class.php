...
class CloudController extends AdminController
{
    ...
    public function getVersionList()
    {
        ...
        $versionList = M('Admin/Cloud')->getUpdateList($aToken);
...
str
    ...
    public function updb()
    {
        ...
        if (IS_POST) {
            if (!file_exists($sql_path)) {
                ...
                $result = M('')->executeSqlFile($sql_path);
                if ($result) {
    ...
    {
        $cloudModel = M('Admin/Cloud');
        switch ($goods['entity']) {
            case 1:
                //todo 插件关联升级数据
                break;
            case 2:
                $goodsInfo = M('Common/Module')->getModule($goods['etitle']);
                $goodsInfo = $cloudModel->getVersionInfo($goodsInfo);
                break;
            case 3:
                $goodsInfo = M('Common/Theme')->getTheme($goods['etitle']);
                $goodsInfo = $cloudModel->getVersionInfo($goodsInfo);
                break;
        }
...
str
    ...
    private function installPlugin($version, $token)
    {
        ...
        $rs = M('Addons')->install($plugin['name']);
        if ($rs === true) {
