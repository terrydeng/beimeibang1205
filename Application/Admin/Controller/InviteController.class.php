...
class InviteController extends AdminController
{
    ...
    {
        parent::_initialize();
        $this->inviteModel=M('Ucenter/Invite');
        $this->inviteTypeModel=M('Ucenter/InviteType');
        $this->inviteBuyLogModel=M('Ucenter/InviteBuyLog');
        $this->inviteLogModel=M('Ucenter/InviteLog');
        $this->inviteUserInfoModel=M('Ucenter/InviteUserInfo');
    }
    ...
    {
        $role_option=M('Role')->where(array('status'=>1))->order('sort asc')->field('id,title')->select();
        return $role_option;
    }
    ...
    {
        $role_option=M('AuthGroup')->where(array('status'=>1))->field('id,title')->select();
        return $role_option;
    }
    ...
    {
        $score_option=M('UcenterScoreType')->where(array('status'=>1))->field('id,title')->select();
