...
class AdminController extends Controller
{
    ...
    final public function getMenus($controller = CONTROLLER_NAME)
    {
        ...
        if (empty($menus)) {
            ...
            $current = D('Menu')->where("url like '{$controller}/" . ACTION_NAME . "%' OR url like '%/{$controller}/" . ACTION_NAME . "%'  ")->field('id')->find();
            if ($current) {
                $nav = M('Menu')->getPath($current['id']);
    ...
    {
        if (C('AUTO_UPDATE')) {
            $can_update = M('Version')->checkUpdate();
        } else {
            $can_update = 0;
        }
