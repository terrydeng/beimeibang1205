...
{
    return $model = D('Model')->getFieldById($id, 'title');
}
...
function lists_plus(&$data)
{
    ...
    foreach ($data as $key => $value) {
        ...
        $mid = D('action_log')->field("max(create_time),remark")->where('action_id=' . $data[$key]['id'])->select();
        $mid_s = $mid[0]['remark'];
...
function get_parent_category($cid)
{
    ...
    $cates = D('Category')->where(array('status' => 1))->field('id,title,pid')->order('sort')->select();
...
function get_type_bycate($id = null)
{
    ...
    $model_type = D('Category')->getFieldById($id, 'type');
...
function get_cate($cate_id = null)
{
    ...
    $cate = D('Category')->where('id=' . $cate_id)->getField('title');
    return $cate;
}
...
{
    return D('Document')->where('pid=' . $id)->count();
}
...
function get_action($id = null, $field = null)
{
    ...
    if (empty($list[$id])) {
        ...
        $list[$id] = D('Action')->where($map)->field(true)->find();
    }
