...
class WeiboModel extends Model
{
	...
	public function getWeiboDetai1l2($id)
    {
        ...
        if ($check_empty) {
            ...
            if ($weibo['type'] === 'feed' || $weibo['type'] == '' || !$class_exists) {
                $fetchConten1t2 = "<p class='word-wrap'>". $weibo['content'] . "</p>";
    ...
    public function getWeiboDetail($id)
    {
        ...
        if ($check_empty) {
			...
			elseif ($weibo['type'] === 'image') {
                //$fetchConten1t2 = A('Weibo/Type')->fetchImag1e2($weibo);
    ...
    public function deleteWeibo($weibo_id)
    {
        ...
        M('Weibo/WeiboComment')->where(array('weibo_id' => $weibo_id))->setField('status', -1);
    ...
    public function getSupportedPeople($weibo_id, $user_fields = array('nickname', 'space_url', 'avatar128', 'space_link'), $num = 8)
    {
        ...
        $supported = M('Support')->getSupportedUser('Weibo', 'weibo', $weibo_id, $user_fields, $num);
