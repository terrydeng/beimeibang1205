...
class WeiboCommentModel extends Model
{
    ...
    public function addComment($uid, $weibo_id, $content, $comment_id = 0)
    {
        ...
        //增加轻博客评论数量
        M('Weibo/Weibo')->where(array('id' => $weibo_id))->setInc('comment_count');
    ...
    {
        //获取轻博客编号
        $comment = M('Weibo/WeiboComment')->find($comment_id);
        if ($comment['status'] == -1) {
            return false;
        }
        ...
        //将评论标记为已经删除
        M('Weibo/WeiboComment')->where(array('id' => $comment_id))->setField('status', -1);

        //减少轻博客的评论数量
        M('Weibo/Weibo')->where(array('id' => $weibo_id))->setDec('comment_count');
        S('weibo_' . $weibo_id, null);
        //返回成功结果
        return true;
    }
