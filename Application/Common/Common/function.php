...
// +----------------------------------------------------------------------
// | Copyright (c) 2015 http://www.onethink.cn All rights reserved.
// +----------------------------------------------------------------------
// | Author: aw34 <1114148@vip.qq.com> <http://www.northsoft.cc>
// +----------------------------------------------------------------------
...
function check_auth($rule = '', $except_uid = -1, $type = AuthRuleModel::RULE_URL)
{
    ...
    // 检测是否有该权限
    if (!D('auth_rule')->where(array('name' => $rule, 'status' => 1))->find()) {
        return false;
    }
   static $Auth = null;
    if (!$Auth) {
...
{
    $rule = M('AuthRule')->where(array('name' => $rule))->find();
    ...
    foreach ($groups as $v) {
        ...
        if (in_array($rule['id'], $auth_rule)) {
            $gid = $v['id'];
            $temp_uids =(array) M('AuthGroupAccess')->where(array('group_id' => $gid))->getField('uid');
            if ($temp_uids !== null) {
...
function get_document_model($id = null, $field = null)
{
    ...
    if (empty($list)) {
        ...
        $model = D('Model')->where($map)->field(true)->select();
...
function action_log($action = null, $model = null, $record_id = null, $user_id = null)
{
    ...
    //查询行为,判断是否执行
    $action_info = D('Action')->getByName($action);
...
function execute_action($rules = false, $action_id = null, $user_id = null, $log_id = null)
{
    ...
    $action_log = D('ActionLog')->where(array('id' => $log_id))->find();
    foreach ($rules as $rule) {
        ...
        $sType = M('ucenter_score_type')->where(array('id' => $rule['field']))->find();
    ...
    if ($log_score) {
        ...
        D('ActionLog')->where(array('id' => $log_id))->setField('remark', array('exp', "CONCAT(remark,'" . $log_score . "')"));
    }
    return $return;
}
...
function get_model_attribute($model_id, $group = true)
{
    ...
    if (!isset($list[$model_id])) {
        ...
        $extend = D('Model')->getFieldById($model_id, 'extend');

        if ($extend) {
            $map = array('model_id' => array("in", array($model_id, $extend)));
        }
        $info = D('Attribute')->where($map)->select();
        $list[$model_id] = $info;
    ...
    if ($group) {
        $sort = D('Model')->getFieldById($model_id, 'field_sort');
...
function get_table_field($value = null, $condition = 'id', $field = null, $table = null)
{
    ...
    //拼接参数
    $map[$condition] = $value;
    $info = D(ucfirst($table))->where($map);
    if (empty($field)) {
...
function get_list_count($category, $status = 1)
{
    ...
    if (!isset($count[$category])) {
        $count[$category] = M('Document')->listCount($category, $status);
    }
...
function get_userdata_join($id = null, $field = null, $table = null)
{
    ...
    if (empty($id)) {
        ...
        if (is_array($id)) {
            $map['id'] = array('in', $id);
            $data = M($table)->where($map)->getField($field, true);
            return implode(',', $data);
        } else {
            $map['id'] = $id;
            $data = M($table)->where($map)->getField($field);
            return $data;
        }
    }
}
...
function get_data_field_id($map = null, $field = null, $table = null, $yesnoid = '')
{
    ...
    if (empty($map)) {
        ...
        if (empty($yesnoid)) {
            ...
            $list = M($table)->where($map)->getField($field);
        }
        return $list;
    }
}
...
//截取字符段长度151107
function subtex1t2($text, $length)
 {
