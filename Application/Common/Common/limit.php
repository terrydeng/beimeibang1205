...
class ActionLimit
{
    ...
    function checkOne($item)
    {
        ...
        $map['status'] = 1;
        $limitList = M('ActionLimit')->getList($map);
        !empty($item['action']) && $item['action_id'] = D('action')->where(array('name' => $item['action']))->getField('id');
        foreach ($limitList as &$val) {
            $ago = get_time_ago($val['time_unit'], $val['time_number'], $time);
            $item['create_time'] = array('egt', $ago);
            $log = D('action_log')->where($item)->order('create_time desc')->select();
            if (count($log) >= $val['frequency']) {
                ...
                unset($punish);
                if ($val['if_message']) {
                    M('Message')->sendMessageWithoutCheckSelf($item['user_id'], L('_SYSTEM_MESSAGE_'),$val['message_content'],$_SERVER['HTTP_REFERER']);
                }
            }
        }
        unset($val);
...
function get_time_unit($key = null){
    $array = array('second' => '秒', 'minute' => '分', 'hour' => L('_HOUR_'), 'day' => '天', 'week' => '周', 'month' => '月', 'year' => '年');
...
function get_action_name($key){
    ...
    $return = array();
    foreach($key as $val){
        $return[] = M('Action')->where(array('name'=>$val))->getField('title');
    }
    return implode(',',$return);
}