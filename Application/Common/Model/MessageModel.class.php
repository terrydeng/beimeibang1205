...
class MessageModel extends Model
{
    ...
    private function addMessageContent($from_uid, $title, $content, $url, $url_args, $type)
    {
        ...
        $message_id = M('message_content')->add($data_content);
        return $message_id;
    }
    ...
    public function getContent($id)
    {
        ...
        if (empty($content)) {
            $content = M('message_content')->find($id);
            if($content){
    ...
    {
        $messages = M('message')->where('to_uid=' . $uid . ' and  is_read=0  and last_toast!=0')->order('id desc')->limit(99999)->select();
    ...
    {
        M('message')->where('to_uid=' . $uid . ' and  is_read=0')->setField('is_read', 1);
    }
    ...
    {
        $now = time();
        M('message')->where('to_uid=' . $uid . ' and  is_read=0 and last_toast=0')->setField('last_toast', $now);
    }
    ...
    {
        $messages = M('message')->where('to_uid=' . $uid . ' and  is_read=0 ')->order('id desc')->limit(99999)->select();
        foreach ($messages as &$v) {
            $v['ctime'] = friendlyDate($v['create_time']);
            $v['content'] = $this->getContent($v['content_id']);
        }
