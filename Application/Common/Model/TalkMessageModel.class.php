...
class TalkMessageModel extends Model
{
    ...
    public function addMessage($content, $uid, $talk_id)
    {
        ...
        M('Talk')->where(array('id'=>intval($talk_id)))->setField('update_time',time());
    ...
    {
        M('Message')->sendMessage($to_uids,L('_YOU_HAVE_A_NEW_CHAT_MESSAGE_'), L('_DIALOGUE_CONTENT_WITH_COLON_') . op_t($content), 'UserCenter/Message/talk', array('talk_id' => $talk_id) , is_login(), 1);
    }
    ...
    {
        $origin_member = M('Talk')->decodeArrayByRec(explode(',', $talk['uids']));
