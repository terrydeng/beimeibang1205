...
class TalkPushModel extends Model
{
    ...
    public function getAllPush()
    {
        ...
        foreach ($new_talks as &$v) {
            $v['talk'] = M('Talk')->find($v['source_id']);
            $uids = M('Common/Talk')->decodeArrayByRec(explode(',', $v['talk']['uids']));
            $user = M('Common/Talk')->getFirstOtherUser($uids);
    ...
    public function clearTalkPush($uid = 0)
    {
        ...
        M('TalkPush')->where(array('uid' => $uid))->delete();
    }

    public function clearTalkMessagePush($uid = 0)
    {
        $uid = $uid == 0 ? get_uid() : $uid;
        M('TalkMessagePush')->where(array('uid' => $uid))->delete();
    }
}