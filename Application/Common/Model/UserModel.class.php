...
class UserModel
{
    ...
    function query_user($fields = null, $uid = 0)
    {
        ...
        if (in_array('is_following', $fields)) {
            $follow = M('Follow')->where(array('who_follow' => get_uid(), 'follow_who' => $uid))->find();
            $result['is_following'] = $follow ? true : false;
        }
        if (in_array('is_followed', $fields)) {
            $follow = M('Follow')->where(array('who_follow' => $uid, 'follow_who' => get_uid()))->find();
    ...
    private function getAlias($uid)
    {
        ...
        if ($alias === false) {
            //没有缓存
            $alias = '';
            $follow = M('Common/Follow')->getFollow(get_uid(), $uid);//获取关注情况
...
//查询缓存，过滤掉已缓存的字段
    ...
    {
//获取两张用户表格中的所有字段
        $homeFields = D('Member')->getDBFields();
        $ucenterFields = D('UcenterMember')->getDBFields();
        ...
        $ucenterResult = array();
        if ($homeFields) {
            $homeResult = D('Member')->where(array('uid' => $uid))->field($homeFields)->find();
        }
        if ($ucenterFields) {
...
//读取用户名拼音
        if (in_array('pinyin', $fields)) {
            ...
            $result['pinyin'] = M('Pinyin')->pinYin($result['nickname']);
            return $result;
        }
        return $result;
    }
...
//获取用户头衔链接
        if (in_array('rank_link', $fields)) {
            $rank_List = M('rank_user')->where(array('uid' => $uid, 'status' => 1))->select();
            $num = 0;
    ...
    public function getExpandInfo($fields, $uid, $map, $map_field, $result)
    {
        if (in_array('expand_info', $fields)) {
            $map['status'] = 1;
            $field_group = M('field_group')->where($map)->select();
            $field_group_ids = array_column($field_group, 'id');
            $map['profile_group_id'] = array('in', $field_group_ids);
            $fields_list = M('field_setting')->where($map)->getField('id,field_name,form_type,visiable');
