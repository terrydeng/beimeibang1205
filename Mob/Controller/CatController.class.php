...
//dump($aMark);exit;
      ...
      //  dump($entity);exit;
        ...
        S('INFO_LIST', null);
//顶部标题展示
        $toptitle = M('CatEntity')->where(array('name' => $aMark, 'status' => 1))->field('alias')->find();
        $this->setTopTitle($toptitle['alias']);
        $this->assign('mark', $aMark);
//查询不同mark对应的内容
        $list = M('cat_entity')->where(array('name' => $aMark))->field('id')->find();
        $map['entity_id'] = $list['id'];
        $map['status'] = 1;
     //   dump($map);exit;
        $list = M('Cat')->getList($map, $aPage, $aCount);
        // dump($list);exit;
        //dump($list[0]);exit;
        //查看更多是否显示，pid为1时显示
        ...
        //发布信息内容渲染
        $type = M('CatEntity')->where(array('status' => 1))->select();
        foreach ($type as &$v) {
            $name = 'add' . $v['name'];
    ...
    {
        $entity_list = M('CatEntity')->where(array('status' => 1))->select();
    ...
    {
        $entity_list = M('CatEntity')->where(array('status' => 1))->field('alias,name')->select();
        foreach ($entity_list as &$v) {
    ...
    public function addMoreList()
    {
        ...
        if($aMyText=='myCollect'){
            $catFav=M('catFav')->where(array('uid'=>is_login()))->field('info_id')->select();
    ...
    public function detail($info_id = "")
    {
        ...
        if (is_login()) {
            ...
            $has_read = M('cat_read')->where($map_read)->count();
            if ($has_read) {
                M('cat_read')->where($map_read)->setField('cTime', time());
            } else {
                $map_read['cTime'] = time();
                M('cat_read')->add($map_read);
            }
        }
        ...
        $read = M('cat_read')->where($map)->order('cTime desc')->limit(10)->select();;
        ...
        /*最近访问的人内容结束*/
        /*新增阅读数，并更改标题*/
        $info = M('cat_info')->find(I('get.info_id', 0, 'intval'));
        // dump($info);exit;
        $info['fav'] = M('CatFav')->where(array('info_id' => $info_id))->count();//获得收藏数
        ...
        $info['rate'] = M('CatRate')->where(array('info_id' => $info_id))->field('score')->select();
        $info['rate'] = array_column($info['rate'], 'score');
        $info['ratecount'] = M('CatRate')->where(array('info_id' => $info_id))->count();
        ...
        $info['read']++;
        M('cat_info')->save($info);    //新增阅读数

        // $entity = M('cat_entity')->find($info['entity_id']);;


        //取出全部的字段数据
        //    $map_field['entity_id'] = $entity['id'];
        $map_field['status'] = 1;
        $fields = M('cat_field')->where($map_field)->order('sort desc')->select();
        //确定是否过期


        //获取到信息的数据
        $detail = M('Data')->getByInfoId($info['id']);
        $info['id'] = $info_id;
...
//打分
    public function doScore()
    {
        ...
        $rs = M('cat_rate')->add($rate);


        if ($rs) {
            $map_rate['info_id'] = $info_id;
            $count = M('cat_rate')->where($map_rate)->Avg('score');
            $map_info['id'] = I('post.info_id', 0, 'intval');
            M('cat_info')->where($map_info)->setField('rate', $count);
    ...
    {
        if ($edit) {
            $detail = M('Data')->getByInfoId($infoId);
            $info = M('cat_info')->find($infoId);
            // dump($detail);exit;
            $this->assign('info', $info);
            $this->assign('detail', $detail);
        }
        $map['name'] = array('in', array('reward', 'place'));
        $selcet = M('CatField')->where(array('status' => 1, $map))->select();
        $field_list_l = array_column($selcet, 'name');
        $selcet = array_combine($field_list_l, $selcet);
        foreach ($selcet as &$v) {
            $v['option'] = explode("\n", $v['option']);
        }
        $alias = M('CatEntity')->where(array('id' => $entity_id))->field('alias')->find();
...
//其他自定义发布页面数据渲染。
    ...
    public function delInfo()
    {
        ...
        $info = M('cat_info')->find($map['info_id']);
        $this->checkAuth('Cat/Index/delInfo', $info['uid'], '你没有删除分类信息的权限！');

        $rs = M('cat_info')->where(array('id' => I('post.info_id', 0, 'intval')))->delete();
        if ($rs) {
            M('cat_data')->where($map)->delete();
            M('cat_com')->where($map)->delete();
        }
        if ($rs) {
    ...
    public function userContent()
    {
        ...
        $sendMsg = M('catInfo')->where(array('uid' => is_login(), 'status' => 1))->count();
        $favdMsg = M('catFav')->where(array('uid' => is_login()))->count();
...
//查询不同mark对应的内容
    ...
    public function myMailBox(){
        ...
        //收到的信息数据渲染
        $rec=M('catSend')->where(array('rec_uid'=>is_login()))->order('create_time desc')->select();
        foreach($rec as &$v){
            $v['info']=M('catInfo')->where(array('id'=>$v['info_id']))->find();
            $v['user']= query_user(array('uid', 'nickname', 'space_mob_url', 'avatar64'), $v['send_uid']);
        }unset ($v);
        $this->assign('rec',$rec);

        //发送的信息数据渲染
        $send=M('catSend')->where(array('send_uid'=>is_login()))->order('create_time desc')->select();
        foreach($send as &$a){
            $a['info']=M('catInfo')->where(array('id'=>$a['info_id']))->find();
            $a['user']= query_user(array('uid', 'nickname', 'space_mob_url', 'avatar64'), $a['send_uid']);
        }unset ($a);
        $this->assign('read',$send);

        //发布信息数据渲染
        $follows = M('Follow')->where(array('who_follow' => is_login(), 'follow_who' => is_login(), '_logic' => 'or'))->limit(999)->select();
        $uids = array();
        ...
        foreach ($uids as $uid) {
            ...
            $user['search_key'] = $user['nickname'] . M('PinYin')->Pinyin($user['nickname']);
            $users[] = $user;
        }
        ...
        foreach($tree as &$v){
            $v['_']=M('catInfo')->where(array('entity_id'=>$v['id']))->select();
        }
       // dump($tree);exit;
    ...
    {
        $issues = M('catInfo')->where(array('entity_id' => $pid, 'status' => 1,'uid'=>is_login()))->limit(999)->select();
    ...
    public function doSendInfo()
    {
        ...
        foreach ($array as $v) {
            if ($array != '') {
                     ...
                     */
                    M('Common/Message')->sendMessage($v,"分类信息发送消息", $tip , 'Cat/Center/rec',array(), get_uid(), 1);
              //  }
            }
        }
        if ($rs) {
    ...
    public function doSearch()
    {
        ...
        if (isset($data['map']) && count($data['map'])) {
            ...
            foreach ($map_params as $param_name => $param_value) {
                ...
                $search_field = M('cat_field')->where($field_map)->find();
                ...
                $cat_data = M('cat_data')->where($data_map)->select();
                //更新info_ids
                if (count($cat_data) == 0) {
                    $data['stutus'] = 0;
                    $data['info'] = '搜索不到相应数据';
...
/*            if ($filted_ids == '') {
        ...
        //获取数据
        if ($this->_type == 'list') {
            $infos = M('Info')->getList($map, $num, $order);
            $info_count = count($infos['data']);
        } else {
            $infos = M('Info')->getLimit($map, $num, $order);
...
if($infos['data']){
    ...
    private function getEntity($data)
    {
        if (intval($data['entity_id']) != 0) {
            //获取预置的模板
            $entity = M('cat_entity')->find(intval($data['entity_id']));
            return $entity;
        } else {
            //通过name查到entity,和entity的id
            $map_t['name'] = $data['name'];
            $entity = M('cat_entity')->where($map_t)->find();
            return $entity;
        }
    }
