...
class UserController extends BaseController
{
    ...
    public function index($uid=null,$tab='weibo')
    {
        ...
        $user_info['tags']=M('Ucenter/UserTagLink')->getUserTag($uid);
    ...
    private  function _myWeibo($uid){
        ...
        $weibo = M('Weibo')->where(array('status' => 1, 'uid'=>$uid))->order('create_time desc')->page($aPage, $aCount)->select();//我关注的人的轻博客
        ...
        foreach ($weibo as &$v) {
            ...
            $totalCount=M('Weibo')->where(array('status' => 1, 'uid'=>$uid))->count();
    ...
    private function _myBlog($uid){
        ...
        $aCount = I('post.count', 10, 'op_t');
        if($uid==is_login()){
            $blog= M('News')->where(array('uid'=>$uid))->order('create_time desc,view desc')->page($aPage, $aCount)->select();
        }else{
            $blog= M('News')->where(array('uid'=>$uid,'status'=>1))->order('create_time desc,view desc')->page($aPage, $aCount)->select();
        }
    ...
    private function _userHead($uid){
        //获得以后头衔
        $user_title['have_title'] = M('RankUser')->where(array('status' => 1, 'uid' => $uid))->select();
        foreach($user_title['have_title'] as &$v){
            $v['title'] = M('Rank')->where(array( 'id' => $v['rank_id']))->find();
            $v['logo']= getThumbImageById($v['title']['logo']);
        }
        //获得待审核头衔
        $user_title['ready_title'] = M('RankUser')->where(array('status' => 0, 'uid' => $uid))->select();
        foreach($user_title['ready_title'] as &$v){
            $v['title'] = M('Rank')->where(array( 'id' => $v['rank_id']))->find();
            $v['logo']= getThumbImageById($v['title']['logo']);
        }
        //获得审核失败头衔
        $user_title['defeat_title'] = M('RankUser')->where(array('status' => -1, 'uid' => $uid))->select();
        foreach($user_title['defeat_title'] as &$v){
            $v['title'] = M('Rank')->where(array( 'id' => $v['rank_id']))->find();
            $v['logo']= getThumbImageById($v['title']['logo']);
        }
        //申请头衔
        $user_title['apply_title'] = M('RankUser')->where(array( 'uid' => $uid))->select();
        $user_title['apply_title']=array_column($user_title['apply_title'],'rank_id');
        if(empty( $user_title['apply_title'])){
            $user_title['apply_title']=M('Rank')->where(array( 'status' => 1))->select();
        }else{
            $user_title['apply_title'] = M('Rank')->where(array( 'status' => 1,'id'=>array('not in',$user_title['apply_title'])))->select();
        }
    ...
    public function rankVerifyCancel()
    {
        ...
        if (is_login() && $rank_id) {
            ...
            $map['uid'] = is_login();
            $map['status'] = 0;
            $result = M('rank_user')->where($map)->delete();
            if ($result) {
                M('Message')->sendMessageWithoutCheckSelf(is_login(),'取消头衔申请',  '头衔申请取消成功', 'Ucenter/Message/message', array('tab' => 'system'));
                $data['status']=1;
                $data['info']='取消成功';
    ...
    public function verify()
    {
        ...
        $data['status'] = 0;
        if ($rank_user_id) {
            $model = M('rank_user')->where(array('id' => $rank_user_id));
            if (!$model->select()) {
                $this->error('请正确选择要重新申请的头衔');
            }
            $result = M('rank_user')->where(array('id' => $rank_user_id))->save($data);
        } else {
            $result = M('rank_user')->add($data);
        }
        if ($result) {
            M('Message')->sendMessageWithoutCheckSelf(is_login(),'头衔申请', '头衔申请成功,等待管理员审核',  'Ucenter/Message/message', array('tab' => 'system'));
            $data['status']=1;
    ...
    private function _userData($uid){
        $userdata=M('Member')->where(array('uid'=>$uid))->find();
        ...
        if ($userdata['pos_province'] != 0) {
            ...
            $userdata['pos_community'] = M('district')->where(array('id' => $userdata['pos_community']))->getField('name');
        }
      // dump($userdata);exit;
        return $userdata;
    }
    ...
    private function _myFocus($uid){
        $following['follow'] = M('Follow')->getFollowing($uid, $page=1,  array('avatar128', 'uid', 'nickname', 'fans', 'following', 'weibocount', 'space_url', 'title','space_mob_url'));
        $following['fans'] = M('Follow')->getFans($uid, $page=1, array('avatar128', 'uid', 'nickname', 'fans', 'following', 'weibocount', 'space_url', 'title','space_mob_url'));
    //    dump($following);exit;
        return   $following;
    }
    ...
    public function _profile_group_list($uid = null)
    {
        ...
        if($fields_list){
            $fields_group_ids=M('FieldSetting')->where(array('id'=>array('in',$fields_list),'status' => '1'))->field('profile_group_id')->select();
            if($fields_group_ids){
                ...
                $map['status'] = 1;
                $profile_group_list = M('field_group')->where($map)->order('sort asc')->select();
            }
        }
    ...
    public function _info_list($id = null, $uid = null)
    {
        ...
        if (isset($uid) && $uid != is_login()) {
            //查看别人的扩展信息
            $field_setting_list = M('field_setting')->where(array('profile_group_id' => $id, 'status' => '1', 'visiable' => '1','id'=>array('in',$fields_list)))->order('sort asc')->select();
            ...
            $field_setting_list = M('field_setting')->where(array('profile_group_id' => $id, 'status' => '1','id'=>array('in',$fields_list)))->order('sort asc')->select();
