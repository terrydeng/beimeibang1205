...
//渲染资讯
    public function index($mark=0,$title='',$title_id='')
    {
        ...
        switch ($mark){
            case '0':                   //全站资讯
                $totalCount=M('News')->where(array('status' => 1,$map))->count();
                $blog= M('News')->where(array('status' => 1,$map))->order('create_time desc')->page($aPage, $aCount)->select();
                $blog_mark['mark']=0;
                break;
            case '1':               //热点资讯
                $this->setTopTitle('热点资讯');
                $totalCount=M('News')->where(array('status' => 1,$map))->count();
                $blog= M('News')->where(array('status' => 1,$map))->order('create_time desc,view desc')->page($aPage, $aCount)->select();
                $blog_mark['mark']=1;
                break;
            case '2':                  //我的投稿
                $this->setTopTitle('我的投稿');
                $totalCount= M('News')->where(array('uid'=>is_login()))->count();
                $blog= M('News')->where(array('uid'=>is_login()))->order('create_time desc,view desc')->page($aPage, $aCount)->select();
                foreach($blog as &$a){
                switch ($a['status']){
                    case '1':
                        $a['approval']='审核通过';
            ...
            case '3':                       //各级分类资讯
                ...
                $totalCount= M('News')->where(array('status' => 1, 'category' => $title_id,$map))->count();
                $blog = M('News')->where(array('status' => 1, 'category' => $title_id,$map))->page($aPage, $aCount)->order('create_time desc,view desc')->select();
                $blog_mark['mark']=3;
    ...
    public function addMoreBlog(){
        ...
        switch ($aMark){
            ...
            case '2':           //我的投稿
                ...
                break;
            case '3':           //各级分类资讯
                $blog = M('News')->where(array('status' => 1, 'category' => $aTitleId,$map))->page($aPage, $aCount)->order('create_time desc,view desc')->select();
        }
...
//dump($blog_detail);exit;
      ...
      //获得原图
        ...
        M('News')->where(array('id' => $id))->setInc('view');//查看数加1
...
//dump($blog_detail);exit;
    ...
    public function doAddComment(){
        ...
        $result = M('LocalComment')->addBlogComment($uid, $aBlogId, $aContent);
        $title =get_nickname(is_login()). '评论了您';
        M('Common/Message')->sendMessage($aUid,$title, "评论内容：$aContent",  'news/index/detail',array('id' => $aBlogId), is_login(), 1);
        action_log('add_issue_comment', 'local_comment', $result, $uid);

        $blog_comment=M('LocalComment')->where(array('app'=>'News','mod'=>'index','status'=>1,'id'=>$result))->order('create_time desc')->select();
        foreach ($blog_comment as &$v) {
            $v['user'] = query_user(array('nickname', 'avatar32','uid'), $v['uid']);
            $v['cover_url'] = getThumbImageById($v['cover']);
        }
    ...
    public function atComment($id, $user,$uid)
    {
        ...
        $map['id'] = array('eq', $id);
        $blog = M('News')->where(array('status' => 1, $map))->select();
        $blog[0]['user_uid']=$uid;
        // dump($issue);exit;
...
//dump($blog[0]);
    ...
    public function delBlogComment()
    {
       ...
       // dump($aBlogId);exit;
        ...
        $blog_uid = M('News')->where(array('status' => 1, 'id' => $aBlogId))->find();//根据资讯ID查找资讯发送人的UID
        $comment_uid = M('LocalComment')->where(array('status' => 1, 'id' => $aCommentId))->find();//根据评论ID查找评论发送人的UID
        if (!is_login()) {
        ...
        if (is_administrator(get_uid()) || $blog_uid['uid'] == get_uid() || $comment_uid['uid'] == get_uid()) {                                     //如果是管理员，则可以删除评论
            $result = M('LocalComment')->deleteBlogComment($aCommentId);
        }
        if ($result) {
            $return['status'] = 1;
        } else {
    ...
    public function blogType()
    {
        ...
        $level = M('NewsCategory')->where(array('status' => 1, 'pid' => 0))->select();        //查找顶级分类pid=0的
        foreach (  $level as &$a){
            $a['two']=M('NewsCategory')->where(array('status' => 1, 'pid' => $a['id']))->select();
        }
    ...
    public function addBlog($id=0){
        if($id>0){          //根据是否有ID传入判断是否是编辑请求
            ...
            $blog_detail['detail']= M('NewsDetail')->where(array('news_id'=>$id))->find();
            $this->_top_menu_list =array(
                'left'=>array(
                    array('type'=>'back','need_confirm'=>1,'confirm_info'=>'确定要返回？','a_class'=>'','span_class'=>''),
                ),
            ...
            $category=M('News/NewsCategory')->getCategoryList(array('status'=>1,'can_post'=>1),1);
