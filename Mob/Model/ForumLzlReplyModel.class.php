...
class ForumLzlReplyModel extends Model
{
    ...
    private function sendReplyMessage($uid, $post_id, $content, $to_uid, $to_f_reply_id,$result,$p)
    {
        ...
        $count = M('ForumLzlReply')->where($map)->count();
        ...
        M('Message')->sendMessage($to_uid,$title,  $content, 'Forum/Index/detail#'.$to_f_reply_id,array('id' => $post_id,'page'=>$p,'sr'=>$to_f_reply_id,'sp'=>$pageCount), $uid, 2);

    }

    public function delLZLReply($id)
    {
        $lzl = M('ForumLzlReply')->where('id=' . $id)->find();
        $data['is_del']=1;
        CheckPermission(array($lzl['uid'])) && $res = $this->where('id=' . $id)->save($data);
        M('ForumPost')->where(array('id' => $lzl['post_id']))->setDec('reply_count');
    ...
    public function getLZLReplyList($to_f_reply_id, $order, $page=1, $limit=3)
    {
        ...
        if ($list == null) {
            $list = M('forum_lzl_reply')->where('is_del=0 and to_f_reply_id=' . $to_f_reply_id)->order($order)->select();
            foreach ($list as $k => &$v) {
    ...
    private function handleAt($post_id, $to_f_reply_id, $content, $p, $map)
    {
        ...
        $pageCount = ceil($count / $limit);
        //增加轻博客的评论数量
        M('ContentHandler')->handleAtWho($content, 'Forum/Index/detail#'.$to_f_reply_id,array('id' => $post_id, 'page' => $p, 'sr' => $to_f_reply_id, 'sp' => $pageCount) ,'',1);
    }
