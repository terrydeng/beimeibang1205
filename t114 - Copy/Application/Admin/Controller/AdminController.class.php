...
class AdminController extends Controller {
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        ...
        if(empty($menus)){
            ...
            $menus['main']  =   D('Menu')->where($where)->order('sort asc')->field('id,title,url')->select();
            ...
            // 查找当前子菜单
            $pid = D('Menu')->where("pid !=0 AND url like '%{$controller}/".ACTION_NAME."%'")->getField('pid');
            if($pid){
                // 查找当前主菜单
                $nav =  D('Menu')->find($pid);
                if($nav['pid']){
                    $nav    =   D('Menu')->find($nav['pid']);
                }
                foreach ($menus['main'] as $key => $item) {
                    // 获取当前主菜单的子菜单项
                    if($item['id'] == $nav['id']){
                        $menus['main'][$key]['class']='current';
                        //生成child树
                        $groups = D('Menu')->where(array('group'=>array('neq',''),'pid' =>$item['id']))->distinct(true)->getField("group",true);
                        //获取二级分类的合法url
                        $where          =   array();
                        $where['pid']   =   $item['id'];
                        $where['hide']  =   0;
