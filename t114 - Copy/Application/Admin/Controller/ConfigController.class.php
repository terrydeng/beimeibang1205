...
class ConfigController extends AdminController {
    ...
    public function edit($id = 0){
        if(IS_POST){
            ...
            $info = array();
            /* 获取数据 */
            $info = D('Config')->field(true)->find($id);
    ...
    public function group() {
        ...
        $list   =   D("Config")->where(array('status'=>1,'group'=>$id))->field('id,name,title,extra,value,remark,type')->order('sort')->select();
        if($list) {
            $this->assign('list',$list);
        }
        $this->assign('id',$id);
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = D('Config')->where($map)->field('id,title')->order('sort asc,id asc')->select();
            ...
            foreach ($ids as $key=>$value){
                $res = D('Config')->where(array('id'=>$value))->setField('sort', $key+1);
            }
            if($res !== false){
