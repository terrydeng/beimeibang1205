...
class ModelController extends AdminController {
    ...
    public function add(){
        //获取所有的模型
        $models = D('Model')->where(array('extend'=>0))->field('id,title')->select();
    ...
    public function edit(){
        ...
        $fields = D('Attribute')->where(array('model_id'=>$data['id']))->getField('id,name,title,is_show',true);
        $fields = empty($fields) ? array() : $fields;
        // 是否继承了其他模型
        if($data['extend'] != 0){
            $extend_fields  = D('Attribute')->where(array('model_id'=>$data['extend']))->getField('id,name,title,is_show',true);
    ...
    public function del(){
        ...
        foreach ($ids as $value){
            $res = M('Model')->del($value);
            if(!$res){
                break;
            }
        }
        if(!$res){
            $this->error(M('Model')->getError());
        }else{
            $this->success('删除模型成功！');
        }
    }
    ...
    public function generate(){
        if(!IS_POST){
            ...
            $res = M('Model')->generate($table,I('post.name'),I('post.title'));
            if($res){
