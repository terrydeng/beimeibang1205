...
class DocumentModel extends Model{
    ...
    protected function getStatus(){
        ...
        if(empty($id)){	//新增
        	...
        	$check 	=	D('Category')->getFieldById($cate,'check');  	
            $status = 	$check ? 2 : 1;
        }else{				//更新
...
//            $map['category_id'] = array('in', M("Category")->getChildrenId($root['id']));
			$map['category_id'] = array('in', M("Cat0s1")->getChildrenId($root['id']));
        }
    ...
    protected function getLink(){
        ...
        $res = M('Url')->update(array('url'=>$link));
        return $res['id'];
    }
