...
class AdminController extends AddonsController
{
    ...
    public function buildList($page = 1, $r = 20)
    {
        ...
        $list = D('Report')->where($map)->page($page, $r)->order('id asc')->select();
        $reportCount = D('Report')->where($map)->count();
        int_to_string($list);
    ...
    public function deleteReport()
    {
        $ids = I('ids', array());
        $map['id'] = array('in', $ids);
        $result = M('Addons://Report/Report')->where($map)->delete();
        if ($result) {
            $this->success('删除成功', 0);
        } else {
            $this->error('删除失败');
    ...
    public function handleReport()
    {
        ...
        $uid = M('Common/Report')->where($map)->field('uid')->select();
        $uid = array_column($uid, 'uid');
        $name = query_user(array('uid', 'nickname'));

        $result = M('Addons://Report/Report')->where($map)->save($data);

        if ($result) {

            M('Message')->sendMessageWithoutCheckSelf($uid, '您的举报已被处理。',  '处理时间：' . friendlyDate($data['handle_time']) . '。' . '处理状态：' . '已处理' . '。' . '处理结果：' . $data['handle_result'] . '。' . '处理人：' . $name['nickname']);
