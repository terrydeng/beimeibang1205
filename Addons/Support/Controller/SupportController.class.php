...
class SupportController extends AddonsController
{
    ...
    public function doSupport()
    {
        ...
        if (M('Support')->where($support)->count()) {
            ...
            if (M('Support')->where($support)->add($support)) {
                ...
                M('Message')->sendMessage($message_uid,$title = $user['nickname'] . '赞了您。', $user['nickname'] . '给您点了个赞。',  $aJump , array('id' => $row));
    ...
    {
        M('Support')->clearCache($support['appname'], $support['table'], $support['row']);
    }
}
