...
class BaseController extends AddonsController
{
    ...
    public function unBind()
    {
        ...
        $user_info = M('Addons://SyncLogin/Info')->$type($token);

        if ($info = M('sync_login')->field('uid')->where($map)->find()) {
    ...
    private function addData($user_info)
    {
        ...
        M('Member')->addSyncData($uid, $user_info);
    ...
    {
        if (is_mobile()) {
            $rs = M('Mob/Member')->mobileLogin($uid, $remember); //登陆
        } else {
            $rs = M('Member')->login($uid, $remember); //登陆
        }
        if ($rs) {
