...
class UcenterController extends AddonsController
{
    ...
    public function bind()
    {
        ...
        foreach ($addon_config['type'] as &$v) {
            ...
            if ($arr[$v]['is_bind']) {
                ...
                $user_info = M('Addons://SyncLogin/Info')->$arr[$v]['name'](array('access_token'=>$token['oauth_token'],'openid'=>$token['oauth_token_secret']));
    ...
    {
        $check = M('sync_login')->where(array('uid' => $uid, 'type' => $type))->find();
        if ($check) {
            return true;
        }
        return false;
    ...
    public function unbind(){
        ...
        $uid = is_login();
        $res=  M('sync_login')->where(array('uid' => $uid, 'type' => $aType))->delete();
        if($res){
