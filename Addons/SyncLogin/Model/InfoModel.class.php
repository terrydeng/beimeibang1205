...
//登录成功，获取腾讯QQ用户信息
    ...
    public function tencent($token){
        ...
        if($data['ret'] == 0){
            ...
            throw_exception("获取腾讯轻博客用户信息失败：{$data['msg']}");
        }
    }
    ...
    public function sina($token){
        ...
        $data = M('Addons://SyncLogin/Sina')->getUserInfo($token);
        if($data['error_code'] == 0){
            ...
            throw_exception("获取新浪轻博客用户信息失败：{$data['error']}");
        }
    }
    ...
    public function t163($token){
        ...
        if($data['error_code'] == 0){
            ...
            throw_exception("获取网易轻博客用户信息失败：{$data['error']}");
        }
    }
